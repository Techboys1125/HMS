...
export const authService = {
    ...
    async login(credentials : LoginCredentials) {
        ...
        const resAny = response as unknown as Record < string,
            unknown >;
        const authData = (response.data || resAny)as Record < string,
            unknown >;

        const accessToken = (authData.accessToken || resAny.accessToken)as string;
        const refreshToken = (authData.refreshToken || resAny.refreshToken)as string;
    ...
    async refreshToken() {
        ...
        const newAccessToken = response.data ?. accessToken || response.data ?. tokenType || "";

        useAuthStore.updateTokens({accessToken: newAccessToken, refreshToken: storedRefreshToken});
