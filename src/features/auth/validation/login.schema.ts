...
export function patientRegisterSchema(data: PatientRegistrationData): string | null {
  ...
  if (!data.mobile || data.mobile.trim().length < 8) return "Valid mobile number is required";
...
export function otpSchema(data: VerifyResetOTPData): string | null {
  if (!data.otp || data.otp.trim().length !== 6) return "6-digit OTP is required";
  return null;
}

export function resetPasswordSchema(data: ResetPasswordData): string | null {
  if (!data.newPassword || data.newPassword.length < 6) return "Password must be at least 6 characters";
...
export function changePasswordSchema(data: ChangePasswordData): string | null {
  ...
  if (!data.newPassword || data.newPassword.length < 6) return "New password must be at least 6 characters";
  if (data.newPassword !== data.confirmPassword) return "Passwords do not match";
  return null;
}
