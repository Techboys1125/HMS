...
export function FamilyMembersTab({
  ...
  const handleAdd = async () => {
    ...
    const created = await patientsApi.addFamilyMember(patient.mrn, newMember);
    if (created) {
  ...
  const handleDelete = async (memberId: string) => {
    const success = await patientsApi.deleteFamilyMember(patient.mrn, memberId);
    if (success) {
        ...
        <div className="bg-slate-50 border border-[#E5E7EB] rounded-xl p-4 space-y-3">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handleAdd}
              className="px-4 py-2 rounded-lg bg-[#0D47A1] text-white text-xs font-bold hover:bg-[#0c3d8a] transition-colors"
            >
              Add
            </button>
            <button
