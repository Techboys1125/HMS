...
const mockPrescriptions = [
  {
    ...
    meds: [
      ...
      { name: "Atorvastatin 20mg", dosage: "1 tablet at bedtime", duration: "30 days", instructions: "Avoid grapefruit juice" },
    ],
  },
];
...
export function PatientSearchScreen({
  ...
  const [viewingPatientMrn, setViewingPatientMrn] = useState<string | null>(null);
  ...
  const filteredVisits = useMemo(() => {
    return mockVisits.filter(v => {
      ...
      const matchDoctor = visitDoctorFilter === "All Doctors" || v.doctor === visitDoctorFilter;
      const matchDept = visitDeptFilter === "All Departments" || v.department === visitDeptFilter;
  ...
  if (viewingPatientMrn) {
    ...
    const detailedPatientName = currentDetailedPatient?.fullName || currentDetailedPatient?.name || "Sarah Mitchell";
    const detailedPatientMrn = currentDetailedPatient?.mrn || String(currentDetailedPatient?.id || "PT-2024-001");
    const detailedPatientAge = currentDetailedPatient?.age || 34;
    const detailedPatientGender = currentDetailedPatient?.gender || "Female";
    const detailedPatientBloodGroup = currentDetailedPatient?.bloodGroup || "O+";
    const detailedPatientPhone = currentDetailedPatient?.phone || currentDetailedPatient?.mobileNumber || "+1 (555) 234-5678";
    const detailedPatientStatus = currentDetailedPatient?.status || "Active";
    const detailedPatientAssignedDoctor = currentDetailedPatient?.assignedDoctor || "Dr. A. Mehta";
    const detailedPatientVisitCount = currentDetailedPatient?.visitCount || currentDetailedPatient?.totalVisits || 12;
    const detailedPatientLastVisit = currentDetailedPatient?.lastVisitDate || currentDetailedPatient?.lastVisit || "Mar 12, 2024";
    const detailedPatientNextAppointment = currentDetailedPatient?.nextAppointmentDate || "N/A";
    ...
    const mapToScreenPatient = (p: Patient | undefined): ScreenPatient | null => {
      if (!p) return null;
      return {
        id: String(p.id ?? p.mrn),
        name: p.fullName || p.name || "",
        age: p.age || 0,
        gender: p.gender === "MALE" || p.gender === "M" ? "M" : p.gender === "FEMALE" || p.gender === "F" ? "F" : "Other",
        mobile: p.phone || p.mobileNumber || p.mobile || "",
        doctor: p.assignedDoctor || "",
        department: "",
        visitType: "",
        regDate: p.registrationDate || "",
        status: p.status === "ACTIVE" || p.status === "Active" ? "Active" : p.status === "INACTIVE" || p.status === "Inactive" ? "Inactive" : "Discharged",
        photo: p.photo || p.photoUrl,
        emergencyContact: p.emergencyContact ? {
          name: p.emergencyContact.name || p.emergencyContact.contactName || "",
          relationship: p.emergencyContact.relationship || "",
          phone: p.emergencyContact.phone || p.emergencyContact.contactNumber || p.emergencyContact.mobile || "",
        } : undefined,
      };
    };
    ...
    return (
      <div className="flex-1 overflow-y-auto p-6 bg-[#F1F5F9]" style={{ fontFamily: RB }}>
        ...
        {toastMsg && (
          <div className="fixed bottom-6 right-6 z-50 bg-[#111827] text-white text-xs px-4 py-3 rounded-2xl shadow-xl flex items-center gap-2.5 border border-slate-700 animate-in fade-in slide-in-from-bottom-2" style={{ fontFamily: RB }}>
        ...
        {/* DRAWERS */}
        <ProfileBookApptDrawer isOpen={isBookDrawerOpen} onClose={() => setIsBookDrawerOpen(false)} patientName={detailedPatientName} onSuccess={triggerToast} />
        ...
        <EditPatientInformationDrawer isOpen={isEditDrawerOpen} onClose={() => setIsEditDrawerOpen(false)} patient={mapToScreenPatient(currentDetailedPatient)} onSaveSuccess={() => triggerToast('Demographic information updated.')} />
        ...
        <div className="w-full space-y-6">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <button onClick={handleBackDetails} className="p-1.5 -ml-1.5 text-slate-400 hover:text-[#0D47A1] hover:bg-blue-50 rounded-lg transition-colors">
                <ChevronLeft size={20} />
              </button>
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-500 pl-8" style={{ fontFamily: RB }}>
              ...
              <button onClick={handleBackDetails} className="hover:text-[#0D47A1] transition-colors">Patient Management</button>
              <ChevronRight size={13} className="text-slate-300" />
              <span className="font-semibold text-[#111827]">{detailedPatientName}</span>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex flex-col sm:flex-row sm:items-center gap-5">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-3 mb-1">
                  <h2 className="text-lg font-bold text-[#111827]" style={{ fontFamily: PP }}>{detailedPatientName}</h2>
                  <span className="text-[10px] font-mono font-semibold text-[#0D47A1] bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100">{detailedPatientMrn}</span>
                ...
                <div className="flex flex-wrap items-center gap-y-1 gap-x-4 text-xs text-slate-600 mt-1" style={{ fontFamily: RB }}>
                  <span className="flex items-center gap-1.5 font-medium"><UserCheck size={14} className="text-slate-400" /> {detailedPatientAge} Y / {detailedPatientGender}</span>
                  <span className="flex items-center gap-1.5 font-medium"><Droplets size={14} className="text-red-500" /> Blood {detailedPatientBloodGroup}</span>
                  <span className="flex items-center gap-1.5 font-medium"><Phone size={14} className="text-slate-400" /> {detailedPatientPhone}</span>
                </div>
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-400 mt-2">
                  ...
                  <span className="flex items-center gap-1"><Stethoscope size={13} className="text-[#009688]" /> {detailedPatientAssignedDoctor}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 shrink-0">
              ...
              {(role === 'admin' || role === 'super-admin') && (
                <>
                  <button
                    onClick={() => (onEditPatientClick ? onEditPatientClick(currentDetailedPatient || ({} as Patient)) : setIsEditDrawerOpen(true))}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm text-center">
              ...
              <div className="text-xl font-bold text-[#111827]" style={{ fontFamily: PP }}>{detailedPatientVisitCount}</div>
            </div>
            <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm text-center">
              <div className="text-[11px] text-slate-500 font-medium mb-0.5">Last Visit</div>
              <div className="text-xs font-bold text-[#111827] mt-1">{detailedPatientLastVisit}</div>
            </div>
            <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm text-center">
              <div className="text-[11px] text-slate-500 font-medium mb-0.5">Upcoming</div>
              <div className="text-xs font-bold text-[#0D47A1] mt-1">{detailedPatientNextAppointment}</div>
            </div>
            <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm text-center">
                  ...
                  <div className="bg-slate-50/60 p-5 rounded-2xl border border-gray-100 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      ...
                      <div>
                        ...
                        <span className="font-semibold text-[#111827] mt-0.5 block">{currentDetailedPatient?.email || "sarah.m@example.com"}</span>
                      </div>
                      <div>
