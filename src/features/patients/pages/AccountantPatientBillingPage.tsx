...
export function AccountantPatientBillingPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    patientApi.listPatients({ status: "ACTIVE" })
      .then((response) => {
        if (!cancelled) {
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-[#F1F5F9]" style={{ fontFamily: RB }}>
        <div className="max-w-4xl mx-auto space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm p-4 mb-4">
            ...
            <p className="text-xs text-[#64748B]">MRN: {selectedPatient.mrn} · {selectedPatient.gender} · {selectedPatient.age} yrs</p>
          </div>
          <PatientBillingTab patient={selectedPatient} canEdit={true} isOwnProfile={false} />
        </div>
      </div>
    );
  }
    ...
    <div className="flex-1 overflow-y-auto p-6 bg-[#F1F5F9]" style={{ fontFamily: RB }}>
      <div className="max-w-4xl mx-auto space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-lg font-bold text-[#111827]" style={{ fontFamily: PP }}>Patient Billing</h1>
        </div>

        <div className="flex items-center gap-3">
          <div className="relative flex-1">
            <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
            <input type="text" placeholder="Search MRN, name, phone..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-3 py-2 text-xs bg-white border border-[#E5E7EB] rounded-xl text-[#111827] outline-none focus:border-[#0D47A1]" />
          </div>
        </div>

        {loading ? (
          <div className="flex items-center justify-center p-8 text-xs text-[#64748B]">Loading patients...</div>
        ) : (
          <div className="space-y-2">
            ...
            {filtered.length === 0 && (
              <div className="text-center py-8 text-xs text-[#64748B]">No patients found.</div>
            )}
          </div>
        )}
      </div>
