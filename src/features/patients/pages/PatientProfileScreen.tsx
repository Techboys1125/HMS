...
export function PatientProfileScreen({
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "appointments" | "visits" | "medical-history" | "billing" | "documents"
  >("overview");
  ...
  const [error, setError] = useState<string | null>(!patientMrn ? "No patient MRN provided" : null);
  ...
  const displayName = patient?.fullName || patient?.name || patient?.patientName || "Patient";
  const displayAge = patient?.age ?? "";
  const displayGender = patient?.gender || "";
  const displayDob = patient?.dob || patient?.dateOfBirth || "";
  const displayBloodGroup = (patient?.bloodGroup || patient?.blood_type || "").replace(/_/g, " ");
  const displayPhone = patient?.phone || patient?.mobileNumber || patient?.mobile || "";
  const displayEmail = patient?.email || "";
  const displayAddress = typeof patient?.address === "string"
    ? patient.address
  ...
  const displayAssignedDoctor = extractDoctorName(patient) || patient?.assignedDoctor || "";
  const displayPhoto = patient?.photoUrl || patient?.photo || "";

  const emergencyContactName = patient?.emergencyContact?.name || patient?.emergencyContact?.contactName || "";
  ...
  const visitHistory = useMemo(
    () =>
      sortedAppointments.filter(
        (a) =>
          (a.status || a.appointmentStatus || "").toLowerCase() === "completed" ||
          (a.status || a.appointmentStatus || "").toLowerCase() === "checked-in",
      ),
    [sortedAppointments],
  );
  ...
  const upcomingAppointments = useMemo(
    () =>
      sortedAppointments.filter(
        (a) =>
          (a.status || a.appointmentStatus || "").toLowerCase() === "scheduled" ||
  ...
  const lastVisitDate = visitHistory.length > 0 ? visitHistory[0].appointmentDate || visitHistory[0].date || "" : "";
  const nextAppointmentDate = upcomingAppointments.length > 0 ? upcomingAppointments[0].appointmentDate || upcomingAppointments[0].date || "" : "";
  const activePrescriptions = safePrescriptions.filter(
    (p) => (p.status || "").toLowerCase() === "issued" || (p.status || "").toLowerCase() === "active",
  ).length;
  const unpaidBilling = safeBilling.filter(
    (b) => (b.status || "").toLowerCase() !== "paid" && (b.status || "").toLowerCase() !== "voided",
  ).length;

  const [apptPage, setApptPage] = useState(1);
  const apptPageSize = 10;
  const apptTotalPages = Math.max(1, Math.ceil(sortedAppointments.length / apptPageSize));
  ...
  const visitTotalPages = Math.max(1, Math.ceil(visitHistory.length / visitPageSize));
  ...
  const billingTotalPages = Math.max(1, Math.ceil(billing.length / billingPageSize));
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-[#F1F5F9]">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-sm text-slate-500">Loading patient profile...</span>
          </div>
        </div>
      </div>
    );
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-[#F1F5F9]">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="flex flex-col items-center gap-3 text-center">
            ...
            <span className="text-sm text-slate-600 font-medium">{error || "Patient not found"}</span>
            <button
              onClick={onBack}
      ...
      <div className="w-full space-y-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-1.5 text-xs text-slate-500 pl-8" style={{ fontFamily: RB }}>
            ...
            <button onClick={onBack} className="hover:text-[#0D47A1] transition-colors">
              Patient Search
            </button>
            <ChevronRight size={13} className="text-slate-300" />
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-5">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3 mb-1">
                <h2 className="text-lg font-bold text-[#111827]" style={{ fontFamily: PP }}>
                  {displayName}
                </h2>
              ...
              <div className="flex flex-wrap items-center gap-y-1 gap-x-4 text-xs text-slate-600 mt-1" style={{ fontFamily: RB }}>
                {displayAge && (
                  <span className="flex items-center gap-1.5 font-medium">
                    <UserCheck size={14} className="text-slate-400" /> {displayAge} Y / {displayGender}
                  </span>
                )}
                {displayBloodGroup && (
                  <span className="flex items-center gap-1.5 font-medium">
                    <Droplets size={14} className="text-red-500" /> {displayBloodGroup}
                  </span>
                )}
                {displayPhone && (
                  <span className="flex items-center gap-1.5 font-medium">
                    <Phone size={14} className="text-slate-400" /> {displayPhone}
                  </span>
                )}
              </div>
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-400 mt-2">
                {displayRegDate && <span>Reg: {formatDisplayDate(displayRegDate)}</span>}
                {displayAssignedDoctor && (
                  <>
                    <span className="hidden sm:inline w-1 h-1 rounded-full bg-slate-300"></span>
                    <span className="flex items-center gap-1">
                      <Stethoscope size={13} className="text-[#009688]" /> {displayAssignedDoctor}
                    </span>
                  </>
                )}
              </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          ...
          <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm text-center">
            ...
            <div className="text-xs font-bold text-[#0D47A1] mt-1">
              {nextAppointmentDate ? formatDisplayDate(nextAppointmentDate) : "—"}
            </div>
          </div>
          <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm text-center">
            <div className="text-[11px] text-slate-500 font-medium mb-0.5">Active Scripts</div>
            <div className="text-xl font-bold text-[#009688]" style={{ fontFamily: PP }}>
              {activePrescriptions}
            </div>
          </div>
          <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm text-center">
            <div className="text-[11px] text-slate-500 font-medium mb-0.5">Outstanding</div>
            <div className={`text-xs font-bold mt-1 ${unpaidBilling > 0 ? "text-amber-600" : "text-green-600"}`}>
              {unpaidBilling > 0 ? `${unpaidBilling} Invoice(s)` : "Cleared"}
            </div>
          </div>
          <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm text-center">
            <div className="text-[11px] text-slate-500 font-medium mb-0.5">Category</div>
            <div className="text-xs font-bold text-slate-700 mt-1">{displayCategory}</div>
          </div>
        </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 min-h-105" style={{ fontFamily: RB }}>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-slate-50/60 p-5 rounded-2xl border border-gray-100 space-y-4">
                      ...
                      <div className="space-y-3 text-xs">
                        ...
                        <div>
                          ...
                          <span className="font-semibold text-[#111827] mt-0.5 block">{displayAddress || "—"}</span>
                        </div>
                        <div>
                          <span className="text-slate-400 block text-[11px]">Email Address</span>
                          <span className="font-semibold text-[#111827] mt-0.5 block">{displayEmail || "—"}</span>
                        </div>
                        <div>
                    ...
                    <div className="bg-slate-50/60 p-5 rounded-2xl border border-gray-100 space-y-4">
                      ...
                      <div className="space-y-3 text-xs">
                        <div>
                          ...
                          <span className="font-semibold text-[#111827] mt-0.5 block">
                            {displayRegDate ? formatDisplayDate(displayRegDate) : "—"}
                          </span>
                        </div>
                        <div>
                          <span className="text-slate-400 block text-[11px]">Registration Type</span>
                          <span className="font-semibold text-[#111827] mt-0.5 block">{displayRegType || "—"}</span>
                        </div>
                        <div>
                          <span className="text-slate-400 block text-[11px]">Patient Category</span>
                          <span className="font-semibold text-[#111827] mt-0.5 block">{displayCategory}</span>
                        </div>
                        <div>
                          <span className="text-slate-400 block text-[11px]">Primary Care Doctor</span>
                          <span className="font-semibold text-[#111827] mt-0.5 block">{displayAssignedDoctor || "—"}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  {((patient.knownAllergies && patient.knownAllergies.length > 0) ||
                    ...
                    <div className="bg-slate-50/60 p-5 rounded-2xl border border-gray-100 space-y-3">
                      <h3 className="text-xs font-bold uppercase tracking-wider text-red-500" style={{ fontFamily: PP }}>
                        Medical Alerts
                      </h3>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-xs font-bold uppercase tracking-wider text-slate-600" style={{ fontFamily: PP }}>
                        Recent Appointments Summary
                      </h3>
                      <button
                          ...
                          >
                            <div>
                              <div className="font-bold text-[#111827] text-xs">
                                {typeof a.doctor === "object" ? (a.doctor as { name?: string }).name : a.doctor || a.doctorName || "—"}
                              </div>
                              <div className="text-[11px] text-slate-500">
                                {typeof a.department === "object"
                                  ? (a.department as { departmentName?: string }).departmentName
                                  : a.department || a.departmentName || "—"}{" "}
                                • {formatDisplayDate(a.appointmentDate || a.date || "")}{" "}
                                {a.time || a.startTime ? `(${formatTime(a.time || a.startTime || "")})` : ""}
                              </div>
                            </div>
                            <StatusBadge status={a.status || a.appointmentStatus || ""} />
                          </div>
                        ))}
                      </div>
                    )}
              ...
              {activeTab === "appointments" && (
                <div className="space-y-4">
                  ...
                  {sortedAppointments.length === 0 ? (
                    <p className="text-xs text-slate-400 py-8 text-center">No appointments found</p>
                  ) : (
                    <>
                        ...
                        <table className="w-full text-left text-xs" style={{ fontFamily: RB }}>
                          ...
                          <tbody className="divide-y divide-gray-100 text-[#111827]">
                            {paginatedAppointments.map((item) => {
                              const apptId = String(item.id || item.appointmentId || "");
                              const doctorName = typeof item.doctor === "object" ? (item.doctor as { name?: string }).name : item.doctor || item.doctorName || "—";
                              const deptName = typeof item.department === "object" ? (item.department as { departmentName?: string }).departmentName : item.department || item.departmentName || "—";
                              const apptDate = item.appointmentDate || item.date || "";
                              const apptTime = item.time || item.startTime || "";
                              const apptStatus = item.status || item.appointmentStatus || "";
                              return (
                                <tr key={apptId} className="hover:bg-slate-50 transition-colors">
                                  <td className="px-3 py-3 font-mono font-bold text-[#0D47A1]">{apptId}</td>
                                  <td className="px-3 py-3 font-semibold">{doctorName}</td>
                                  <td className="px-3 py-3 text-slate-600">{deptName}</td>
                                  <td className="px-3 py-3 font-mono text-slate-500">
                                    {formatDisplayDate(apptDate)}{apptTime ? ` · ${formatTime(apptTime)}` : ""}
                                  </td>
                                  <td className="px-3 py-3">
                                    <Chip label={apptStatus} variant={getStatusChipVariant(apptStatus)} />
                                  </td>
                                  <td className="px-3 py-3 text-right">
                                    {apptStatus.toLowerCase() === "scheduled" || apptStatus.toLowerCase() === "confirmed" ? (
                                      <button
                                        onClick={() => onCheckInClick?.(apptId, mrn)}
              ...
              {activeTab === "visits" && (
                <div className="space-y-4">
                  ...
                  {visitHistory.length === 0 ? (
                    <p className="text-xs text-slate-400 py-8 text-center">No visit history found</p>
                  ) : (
                    <>
                      <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse text-xs">
                          <thead>
                            <tr className="bg-slate-50 border-b border-gray-200">
                              {["Visit Date", "Visit ID", "Doctor", "Department", "Status", "Actions"].map((h) => (
                                <th
                                  key={h}
                          ...
                          <tbody className="divide-y divide-gray-100">
                            {paginatedVisits.map((v) => {
                              const visitId = String(v.id || v.appointmentId || "");
                              const doctorName = typeof v.doctor === "object" ? (v.doctor as { name?: string }).name : v.doctor || v.doctorName || "—";
                              const deptName = typeof v.department === "object" ? (v.department as { departmentName?: string }).departmentName : v.department || v.departmentName || "—";
                              const visitDate = v.appointmentDate || v.date || "";
                              const visitStatus = v.status || v.appointmentStatus || "";
                              return (
                                <tr key={visitId} className="hover:bg-blue-50/30 transition-colors">
                                  ...
                                  <td className="px-3.5 py-3 whitespace-nowrap font-mono font-bold text-[#0D47A1]">
                                    <span className="bg-blue-50 px-2 py-0.5 rounded border border-blue-100">{visitId}</span>
                                  </td>
                                  <td className="px-3.5 py-3 whitespace-nowrap font-semibold text-[#111827]">{doctorName}</td>
                                  <td className="px-3.5 py-3 whitespace-nowrap text-slate-600">{deptName}</td>
                                  <td className="px-3.5 py-3 whitespace-nowrap">
                                    <Chip label={visitStatus} variant={getStatusChipVariant(visitStatus)} />
                                  </td>
                                  <td className="px-3.5 py-3 whitespace-nowrap">
                                    <button
                  ...
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-xs space-y-2">
                    ...
                    <p className="text-slate-600 leading-relaxed">
                      {patient.medicalHistory && patient.medicalHistory.length > 0
                        ...
                        : patient.specialNotes || "No medical history recorded."}
                    </p>
                  </div>
                  {prescriptions.length > 0 && (
                    <div className="space-y-2">
                      <h4 className="text-xs font-bold text-slate-600 uppercase tracking-wider" style={{ fontFamily: PP }}>
                        Active Prescriptions
                      </h4>
                      {prescriptions.slice(0, 5).map((rx) => (
                        <div key={String(rx.id)} className="p-3 rounded-xl border border-gray-100 bg-white shadow-sm text-xs">
                          <div className="flex items-center justify-between">
                            <div>
                              <span className="font-bold text-[#111827]">{rx.doctorName || "—"}</span>
                              <span className="text-slate-400 mx-1">•</span>
                              <span className="text-slate-600">{rx.department || "—"}</span>
                              <span className="text-slate-400 mx-1">•</span>
                              <span className="text-slate-500">{formatDisplayDate(rx.date || "")}</span>
                            </div>
                            <StatusBadge status={rx.status || ""} />
                          </div>
                          {rx.diagnosis && <div className="text-slate-500 mt-1">Diagnosis: {rx.diagnosis}</div>}
                        </div>
                      ))}
                    </div>
                  )}
                        ...
                        <table className="w-full text-left text-xs" style={{ fontFamily: RB }}>
                          ...
                          <tbody className="divide-y divide-gray-100 text-[#111827]">
                            {paginatedBilling.map((inv) => (
                              <tr key={String(inv.id)} className="hover:bg-slate-50">
                                ...
                                <td className="px-3 py-3">
                                  <Chip label={inv.status || "—"} variant={getStatusChipVariant(inv.status || "")} />
                                </td>
                              </tr>
                            ))}
                          </tbody>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="p-3 rounded-xl border border-gray-200 bg-slate-50 flex items-center justify-between text-xs">
                      ...
                      <button
                        onClick={() => console.log("Downloading Registration Document...")}
