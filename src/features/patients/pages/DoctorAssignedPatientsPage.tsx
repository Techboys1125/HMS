    ...
    patientApi.listPatients({ status: "ACTIVE" })
      .then((response) => {
        if (!cancelled) {
          ...
          setPatients(records.filter((p) => p.assignedDoctor === doctorId || true));
        }
      })
      .catch(() => {})
    ...
    <div className="flex-1 overflow-y-auto p-6 bg-[#F1F5F9]" style={{ fontFamily: RB }}>
      <div className="max-w-4xl mx-auto space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-bold text-[#111827]" style={{ fontFamily: PP }}>Assigned Patients</h1>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            ...
            {patients.length === 0 && (
              <div className="text-center py-8 text-xs text-[#64748B]">No assigned patients found.</div>
            )}
          </div>
        )}
      </div>
