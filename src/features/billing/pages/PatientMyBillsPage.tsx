...
export function PatientMyBillsPage() {
  ...
  const filteredInvoices = useMemo(() => {
    return invoices.filter((inv) => {
      ...
      const matchesSearch = !q || inv.id.toLowerCase().includes(q) || inv.patientName.toLowerCase().includes(q) || inv.mrn.toLowerCase().includes(q);
  ...
  const summary = useMemo(() => {
    const totalBilled = invoices.reduce((sum, inv) => sum + inv.invoiceAmount, 0);
  ...
  return (
    <div className="w-full bg-[#F1F5F9] min-h-screen p-4 md:p-6 pb-28 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-5 rounded-2xl border border-[#E5E7EB] shadow-sm">
        <div>
          <div className="flex items-center gap-2 text-xs text-[#64748B] mb-1 font-medium" style={{ fontFamily: RB }}>
            <span className="hover:text-[#0D47A1] cursor-pointer" onClick={() => navigate("/dashboard")}>Home</span>
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          <button onClick={() => window.print()} className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-white border border-[#E5E7EB] text-slate-700 text-xs font-semibold hover:bg-slate-50 transition-colors shadow-sm" style={{ fontFamily: RB }}>
            <Printer size={14} /><span className="hidden sm:inline">Print Summary</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-2xl border border-[#E5E7EB] shadow-sm space-y-1">
          ...
          {isLoading ? (
            ...
            <div className="text-xl font-bold text-[#0D47A1]" style={{ fontFamily: PP }}>₹{summary.totalBilled.toLocaleString()}</div>
          )}
          <span className="text-[10px] text-slate-400">{summary.count} invoices</span>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-[#E5E7EB] shadow-sm space-y-1">
          <div className="flex items-center justify-between text-slate-400">
          ...
          {isLoading ? (
            ...
            <div className="text-xl font-bold text-[#66BB6A]" style={{ fontFamily: PP }}>₹{summary.totalPaid.toLocaleString()}</div>
          )}
          <span className="text-[10px] text-[#66BB6A] font-medium">Settled</span>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-[#E5E7EB] shadow-sm space-y-1">
          <div className="flex items-center justify-between text-slate-400">
          ...
          {isLoading ? (
            ...
            <div className="text-xl font-bold text-[#F59E0B]" style={{ fontFamily: PP }}>₹{summary.totalPending.toLocaleString()}</div>
          )}
          <span className="text-[10px] text-amber-600 font-medium">Awaiting payment</span>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-[#E5E7EB] shadow-sm space-y-1">
          <div className="flex items-center justify-between text-slate-400">
            <span className="text-[11px] font-bold uppercase tracking-wider">Payment Rate</span>
          ...
          {isLoading ? (
            ...
            <div className="text-xl font-bold text-[#111827]" style={{ fontFamily: PP }}>
              {summary.totalBilled > 0 ? Math.round((summary.totalPaid / summary.totalBilled) * 100) : 100}%
            </div>
          )}
      ...
      <div className="bg-white p-4 rounded-2xl border border-[#E5E7EB] shadow-sm space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs" style={{ fontFamily: RB }}>
        ...
        <div className="flex items-center justify-end gap-2">
          <button onClick={handleResetFilters} className="px-4 py-2 rounded-xl border border-slate-200 text-slate-600 font-semibold hover:bg-slate-50 transition-colors text-xs">Reset Filters</button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b border-gray-100 pb-3">
          <div>
            <h3 className="text-sm font-bold text-[#111827]" style={{ fontFamily: PP }}>MY INVOICES ({filteredInvoices.length})</h3>
            <p className="text-xs text-[#64748B]" style={{ fontFamily: RB }}>Complete record of your medical billing invoices</p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="py-12 text-center space-y-3">
            ...
            <h4 className="text-sm font-bold text-[#111827]" style={{ fontFamily: PP }}>No invoices found</h4>
            <p className="text-xs text-[#64748B] max-w-sm mx-auto" style={{ fontFamily: RB }}>
              {searchQuery ? "Try adjusting your search or filters." : "You don't have any billing records yet."}
            </p>
            {searchQuery && (
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredInvoices.map((inv) => (
                  <tr key={inv.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="py-3 px-4 text-slate-600 whitespace-nowrap">{inv.invoiceDate}</td>
                    <td className="py-3 px-4 text-slate-700 font-medium">{inv.doctorName}</td>
                    <td className="py-3 px-4 text-right text-slate-700">₹{inv.invoiceAmount.toLocaleString()}</td>
                    <td className="py-3 px-4 text-right font-bold text-[#66BB6A]">₹{inv.paidAmount.toLocaleString()}</td>
                    <td className="py-3 px-4 text-right font-semibold text-[#EF4444]">{inv.balance > 0 ? `₹${inv.balance.toLocaleString()}` : "₹0"}</td>
                    ...
                    <td className="py-3 px-4 text-center">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <button
                          onClick={() => navigate(`/billing/invoice/${inv.id}/print`)}
      ...
      <div className="sticky bottom-0 -mx-4 md:-mx-6 -mb-4 md:-mb-6 bg-white/95 backdrop-blur-md border-t border-[#E5E7EB] p-3.5 px-6 z-40 flex items-center justify-between shadow-lg">
        <button onClick={() => navigate("/dashboard")} className="px-4 py-2 rounded-xl border border-slate-200 text-slate-600 text-xs font-medium hover:bg-slate-100">Back to Dashboard</button>
