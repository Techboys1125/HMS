...
export function InvoiceDetailsPage() {
  ...
  const { paymentHistory, receivePayment, isReceiving } = usePayment(invoiceId);
  ...
  const handleRefund = async () => {
    if (invoiceId && refundAmount > 0 && refundAmount <= paidAmount) {
      await refund({ billId: invoiceId, amount: refundAmount, reason: refundReason });
    ...
    <div className="w-full bg-[#F1F5F9] min-h-screen p-4 md:p-6 pb-28 space-y-6">
      ...
      {actionSuccess && (
        <div className="fixed top-4 right-4 bg-white rounded-xl border border-[#E5E7EB] shadow-lg p-3 flex items-center gap-2 z-50 animate-in slide-in-from-right">
          ...
          <span className="text-xs font-semibold text-[#111827]" style={{ fontFamily: RB }}>{actionSuccess}</span>
        </div>
      )}
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-5 rounded-2xl border border-[#E5E7EB] shadow-sm">
        <div>
          ...
          <p className="text-xs md:text-sm text-[#64748B] mt-0.5" style={{ fontFamily: RB }}>
            Review invoice details, itemizations, payment receipts, and collection transactions.
          </p>
        </div>
          ...
          <div className="relative">
            ...
            {showMoreMenu && (
              <div className="absolute right-0 mt-1 w-44 bg-white rounded-xl border border-[#E5E7EB] shadow-lg py-1 z-30 text-left text-xs" style={{ fontFamily: RB }}>
                ...
                {isDraft && canCancel && (
                  <button onClick={handleCancel} className="w-full px-3 py-2 text-[#EF4444] hover:bg-red-50 flex items-center gap-2 font-medium cursor-pointer">
                    <Ban size={13} />
                    Cancel Invoice
                  </button>
                )}
                {isFinalized && canCancel && (
                  <button onClick={handleVoid} className="w-full px-3 py-2 text-[#F59E0B] hover:bg-amber-50 flex items-center gap-2 font-medium cursor-pointer">
                    <Ban size={13} />
                    Void Invoice
                  </button>
                )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-sm space-y-4">
            <h2 className="text-sm font-bold text-[#111827] border-b border-slate-100 pb-2" style={{ fontFamily: PP }}>Invoice Metadata</h2>
          ...
          {/* Patient Info */}
          <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-sm space-y-4">
            <h2 className="text-sm font-bold text-[#111827] border-b border-slate-100 pb-2" style={{ fontFamily: PP }}>Patient Profile Details</h2>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                ...
                {patientPhone && (
                  <div>
                    ...
                    <span className="font-medium text-[#111827]">{patientPhone}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {/* Doctor Info */}
          <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-sm space-y-4">
            <h2 className="text-sm font-bold text-[#111827] border-b border-slate-100 pb-2" style={{ fontFamily: PP }}>Doctor & OPD Consultation</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-xs" style={{ fontFamily: RB }}>
              <div>
              ...
              {bill.doctor?.doctorCode && (
                <div>
                  ...
                  <span className="font-mono font-semibold text-slate-700">{bill.doctor.doctorCode}</span>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Line Items */}
          <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-sm space-y-4">
            <h2 className="text-sm font-bold text-[#111827] border-b border-slate-100 pb-2" style={{ fontFamily: PP }}>
                ...
                {items.length > 0 ? (
                  items.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td className="py-3 px-3 text-right font-semibold">₹{(item.totalAmount || item.totalPrice || 0).toLocaleString()}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td className="py-3 px-3 text-slate-400" colSpan={4}>No items added to this bill.</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          {paymentRecords.length > 0 && (
            <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-sm space-y-4">
              ...
              <table className="w-full text-left border-collapse text-xs" style={{ fontFamily: RB }}>
                ...
                <tbody className="divide-y divide-slate-100">
                  {paymentRecords.map((p: any, i: number) => (
                    <tr key={i}>
                      <td className="py-3 px-3 font-semibold">{p.method}</td>
                      <td className="py-3 px-3 text-right font-bold text-[#66BB6A]">₹{p.amount.toLocaleString()}</td>
                      <td className="py-3 px-3 text-slate-500 font-mono">{p.referenceNumber || p.paymentNumber || "—"}</td>
                      <td className="py-3 px-3 text-slate-500">{p.receivedBy || "—"}</td>
                      <td className="py-3 px-3 text-slate-500">{p.paidAt || p.transactionDate || "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-sm space-y-4 sticky top-6">
            ...
            <div className="space-y-3 text-xs" style={{ fontFamily: RB }}>
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-slate-800">₹{(summaryData?.grossAmount ?? 0).toLocaleString()}</span>
              </div>
              {(summaryData?.discountAmount ?? 0) > 0 && (
                <div className="flex justify-between text-[#EF4444]">
                  <span>Discount:</span>
                  <span className="font-semibold">-₹{(summaryData?.discountAmount ?? 0).toLocaleString()}</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-slate-500">Net Amount:</span>
                <span className="font-semibold text-slate-800">₹{netAmount.toLocaleString()}</span>
              </div>
          ...
          {isCancelled && (
            <div className="bg-white rounded-2xl border border-red-200 p-5 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-bold text-red-700" style={{ fontFamily: PP }}>Invoice Cancelled</h4>
                  <p className="text-xs text-red-500" style={{ fontFamily: RB }}>This invoice has been cancelled.</p>
                </div>
              </div>
            </div>
          )}
          ...
          {isVoided && (
            <div className="bg-white rounded-2xl border border-amber-200 p-5 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-bold text-amber-700" style={{ fontFamily: PP }}>Invoice Voided</h4>
                  <p className="text-xs text-amber-500" style={{ fontFamily: RB }}>This invoice has been voided.</p>
                </div>
              </div>
            </div>
          )}
          ...
          {!canCollect && !isCancelled && !isVoided && (
            <div className="bg-white rounded-2xl border border-[#66BB6A]/30 p-5 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-bold text-[#66BB6A]" style={{ fontFamily: PP }}>Fully Paid</h4>
                  <p className="text-xs text-slate-500" style={{ fontFamily: RB }}>This invoice is fully settled.</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-2xl w-full max-w-md p-6 space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-sm font-bold text-[#111827]" style={{ fontFamily: PP }}>Process Refund</h3>
              <button onClick={() => setShowRefundModal(false)} className="p-1 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100">✕</button>
            </div>
            <div className="space-y-3 text-xs" style={{ fontFamily: RB }}>
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200">
                <div className="flex justify-between"><span className="text-slate-500">Invoice:</span><span className="font-mono font-bold text-[#0D47A1]">{billData.billNumber}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Paid Amount:</span><span className="font-bold text-[#66BB6A]">₹{paidAmount.toLocaleString()}</span></div>
              </div>
              <div>
                <label className="block text-slate-700 font-semibold mb-1">Refund Amount (₹) *</label>
                <input type="number" value={refundAmount || ""} onChange={(e) => setRefundAmount(Number(e.target.value))} max={paidAmount} className="w-full px-3 py-2.5 rounded-xl border border-[#E5E7EB] bg-slate-50 text-sm font-bold focus:bg-white focus:border-[#0D47A1] focus:outline-none" />
              </div>
              <div>
                <label className="block text-slate-700 font-semibold mb-1">Reason *</label>
                <textarea rows={2} value={refundReason} onChange={(e) => setRefundReason(e.target.value)} placeholder="Enter refund reason..." className="w-full px-3 py-2 rounded-xl border border-[#E5E7EB] bg-slate-50 text-xs focus:bg-white focus:border-[#0D47A1] focus:outline-none" />
              </div>
            </div>
            <div className="flex gap-2 pt-2">
              <button onClick={() => setShowRefundModal(false)} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-600 text-xs font-semibold hover:bg-slate-50">Cancel</button>
              <button onClick={handleRefund} disabled={isRefunding || refundAmount <= 0 || refundAmount > paidAmount} className="flex-1 py-2.5 rounded-xl bg-amber-500 text-white text-xs font-bold hover:bg-amber-600 disabled:opacity-50 flex items-center justify-center gap-2">
                <RotateCcw size={14} />
                {isRefunding ? "Processing..." : `Refund ₹${refundAmount.toLocaleString()}`}
              </button>
            </div>
          </div>
        </div>
