...
export function HolidayCalendar({
  ...
  return (
    <section className="rounded-2xl border border-[#E5E7EB] bg-white p-5 shadow-sm">
      <h3
        style={{ fontFamily: PP }}
        className="mb-4 flex items-center gap-2 text-[15px] font-bold text-[#111827]"
      >
        <Calendar size={18} className="text-[#0D47A1]" /> Section 05: Official
      ...
      <div className="mb-4 grid grid-cols-1 gap-2 md:grid-cols-[1fr_160px_1fr_auto]">
        ...
        />
        <button
          className="inline-flex items-center justify-center gap-1 rounded-lg bg-[#0D47A1] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50"
      ...
      {loading ? (
        ...
        <p className="text-sm text-slate-500">No OPD holidays configured.</p>
      ) : (
            ...
            >
              <div>
                ...
                <div className="text-xs text-[#64748B]">
                  {holiday.holidayDate} · {holiday.holidayType} ·{" "}
                  {holiday.status}
                </div>
                {holiday.description && (
              ...
              <button
                className="inline-flex items-center gap-1 rounded-md border border-[#D1D5DB] bg-white px-2 py-1 text-xs font-semibold text-[#475569]"
                disabled={saving}
                onClick={() => void onToggle(holiday)}
              >
                <Power size={13} />{" "}
