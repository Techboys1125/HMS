...
export const PrescriptionTimeline: React.FC<{
  ...
  return (
    <div className="space-y-4 relative before:absolute before:left-3.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200 pl-8">
      {timeline.map((event) => (
        <div key={event.title} className="relative text-xs">
          <span className="absolute -left-[27px] top-1 w-2.5 h-2.5 rounded-full bg-[#0D47A1] border-2 border-white" />
