...
const ROLES_CONFIG = [
  { id: 'DOCTOR', label: 'Doctor', desc: 'OPD Clinical Portal & Support', icon: Stethoscope },
  { id: 'RECEPTIONIST', label: 'Receptionist', desc: 'Patient Check-in, Queue & Bookings', icon: Users },
  { id: 'NURSE', label: 'Nurse', desc: 'Outpatient care, Vitals & Queue', icon: HeartPulse },
  { id: 'ACCOUNTANT', label: 'Accountant', desc: 'Billing, Payments & Refunds', icon: CreditCard },
];
          ...
          {ROLES_CONFIG.map(role => {
            ...
            return (
              ...
              >
                ...
                <div>
                  <h4 className="font-heading text-xs font-bold text-slate-900 uppercase tracking-wide">{role.label}</h4>
                  <p className="font-body text-[10px] text-slate-400 font-medium leading-relaxed mt-0.5">{role.desc}</p>
                </div>
              </div>
            );
          })}
        </div>
        {errors.role && <p className="text-red-500 text-[10px] font-semibold mt-0.5">{errors.role}</p>}
      </div>
    </div>
  );
};
