...
import { Phone, Mail, MapPin, GraduationCap, Stethoscope, Hash, Tag } from "lucide-react";
...
export function ProfileTab({ doctor, isOwnProfile, canEdit, onSave }: ProfileTabProps) {
  ...
  const allowedFields = isOwnProfile
    ...
    : ["phone", "email", "qualification", "consultationFee", "experienceYrs", "department", "specialty", "bio"] as const;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div>
          <label className="block text-[11px] font-bold text-[#64748B] mb-1">Employee ID</label>
          <div className={fieldStyle(false)}>{formData.empId}</div>
        </div>
        <div>
          <label className="block text-[11px] font-bold text-[#64748B] mb-1">Registration Number</label>
          <div className={fieldStyle(false)}>{formData.regNumber}</div>
        </div>
        <div>
          <label className="block text-[11px] font-bold text-[#64748B] mb-1">Doctor ID</label>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div>
          <label className="block text-[11px] font-bold text-[#64748B] mb-1">
            <GraduationCap size={12} className="inline mr-1" />Qualification
          </label>
        ...
        <div>
          <label className="block text-[11px] font-bold text-[#64748B] mb-1">
            <Stethoscope size={12} className="inline mr-1" />Experience
          </label>
          {isFieldEditable("experienceYrs") ? (
            <input
              ...
              onChange={(e) => setFormData({ ...formData, experienceYrs: Number(e.target.value) })}
              className={fieldStyle(true)}
            />
          ) : (
            <div className={fieldStyle(false)}>{formData.experienceYrs} years</div>
          )}
        </div>
        <div>
          <label className="block text-[11px] font-bold text-[#64748B] mb-1">
            <Tag size={12} className="inline mr-1" />Department
          </label>
        ...
        <div>
          <label className="block text-[11px] font-bold text-[#64748B] mb-1">
            <Hash size={12} className="inline mr-1" />Specialty
          </label>
      ...
      <div>
        <label className="block text-[11px] font-bold text-[#64748B] mb-1">Bio</label>
        {isFieldEditable("bio") ? (
          <textarea
            rows={3}
            value={formData.bio || ""}
      ...
      {editing && (
        <div className="flex items-center gap-2 pt-2">
          ...
          <button
            type="button"
            onClick={() => { setFormData(doctor); setEditing(false); }}
