...
import type { AdminCreateStaffData, AdminUpdateStaffData } from "../../users/types/users.types";
...
const normalizeNumericId = (id: string): string => {
  const cleaned = String(id ?? "").replace(/^DOC-/, "").trim();
  return cleaned || id;
};
...
export function useDoctors() {
  ...
  const addDoctor = useCallback(async (doctor: DoctorRecord) => {
    setLoading(true);
    try {
      const { mapDoctorRecordToCreatePayload } = await import("../api/mapApiUserToDoctorRecord");
      const payload = mapDoctorRecordToCreatePayload(doctor) as unknown as AdminCreateStaffData;
  ...
  const updateDoctor = useCallback(async (id: string, updates: Partial<DoctorRecord>) => {
    setLoading(true);
    try {
      ...
      const { mapDoctorToUpdatePayload } = await import("../api/mapApiUserToDoctorRecord");
      const payload = mapDoctorToUpdatePayload({ ...existing, ...updates }) as unknown as AdminUpdateStaffData;
  ...
  const deactivateDoctor = useCallback(async (id: string) => {
    ...
    try {
      ...
      setDoctors((prev) =>
        prev.map((d) =>
          d.id === id
            ? { ...d, status: "Inactive" as DoctorStatus, availability: "Out of Office" as DoctorAvailability }
            : d,
        ),
      );
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Failed to deactivate doctor";
      setError(msg);
      throw err;
    } finally {
      setLoading(false);
  ...
  const reactivateDoctor = useCallback(async (id: string) => {
    ...
    try {
      ...
      setDoctors((prev) =>
        prev.map((d) =>
          d.id === id
            ? { ...d, status: "Active" as DoctorStatus, availability: "Available Today" as DoctorAvailability }
            : d,
        ),
      );
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Failed to activate doctor";
      setError(msg);
      throw err;
    } finally {
      setLoading(false);
