...
const toDisplayStatus = (status? : string) : AppointmentRecord["status"] => STATUS_MAP[String(status || "").toUpperCase()] || (status as AppointmentRecord["status"]) || "Scheduled";
...
export const normalizeAppointmentRecord = (item : any) : AppointmentRecord => {
    ...
    const startTime = item ?. startTime || item ?. timeSlot || item ?. appointmentTime || "";
...
export const appointmentService = {
    ...
    async listDoctorAppointments(date? : string, status? : string,): Promise < AppointmentRecord[] > {
        ...
        const items = unwrapAppointmentCollection(res);
        return items.filter((item) => !status ? true : String(item ?. status || "").toUpperCase() === status.toUpperCase(),).map(normalizeAppointmentRecord);
    },
    ...
    async rescheduleAppointment(appointmentId : string | number, payload : RescheduleAppointmentRequest,): Promise < AppointmentRecord > {
    ...
    async cancelAppointment(appointmentId : string | number, payload : CancelAppointmentRequest,): Promise < AppointmentRecord > {
...
export typeAppointmentService = typeof appointmentService;

export const isUserRole = (role? : string | null) : role is UserRole => Boolean(role) && [
    "Receptionist",
    "Admin",
    "Hospital Admin",
    "Super Admin",
