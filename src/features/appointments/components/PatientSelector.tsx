...
import { User, Users, PlusCircle, CheckCircle2, ShieldCheck, Heart } from 'lucide-react';
    ...
    try {
      ...
      if (res.data && res.data.length > 0) {
        ...
        if (!selectedPatientId) {
          const selfPatient = res.data.find(p => p.relationship === 'SELF') || res.data[0];
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <label className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5" style={{ fontFamily: PP }}>
          <Heart size={14} className="text-rose-500" />
          Who is this appointment for? *
        </label>
        <span className="text-[10px] text-slate-400 font-medium">User ≠ Patient Architecture</span>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
        {patients.map((p) => {
          const isSelected = String(selectedPatientId) === String(p.patientId) || String(selectedPatientId) === String(p.id);
          return (
            <div
              key={p.id}
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2.5">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${isSelected ? 'bg-[#0D47A1] text-white' : 'bg-slate-100 text-slate-600'
                    }`}>
                    {p.relationship === 'SELF' ? <User size={15} /> : <Users size={15} />}
                  </div>
                  <div>
                    <h4 className="text-xs font-bold text-slate-900" style={{ fontFamily: PP }}>
                      {p.fullName}
                    </h4>
              ...
              <div className="mt-2.5 flex items-center justify-between border-t border-slate-100 pt-2 text-[10px]">
                <span className={`px-2 py-0.5 rounded-md font-semibold border ${getRelationshipBadge(p.relationship)}`}>
                  {p.relationship}
                </span>
                {p.gender && <span className="text-slate-400 font-medium">{p.gender}</span>}
              </div>
            </div>
          );
        })}
      ...
      <p className="text-[11px] text-slate-500 font-medium flex items-center gap-1">
        ...
        Appointments will be issued directly to the selected patient's official hospital record.
      </p>
    </div>
  );
};
