...
type AppointmentStatus = "Scheduled" | "Checked-In" | "Waiting" | "In Progress" | "Completed" | "Cancelled";
...
export function StatusBadge({ status }: { status: AppointmentStatus | string }) {
  const c = STATUS_CONFIG[status as AppointmentStatus] || STATUS_CONFIG["Scheduled"];
  return (
    <span
