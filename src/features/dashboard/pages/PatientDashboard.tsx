...
export function PatientDashboard() {
  ...
  useEffect(() => {
    let isMounted = true;
    appointmentsApi
      .getPatientAppointments("PAT-101")
      .then((res) => {
        if (
          isMounted &&
          res.data &&
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-[#F1F5F9]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          <button
            className="flex items-center gap-2 px-4 py-2 rounded-xl border border-[#E5E7EB] bg-white text-xs font-semibold text-[#111827] hover:bg-slate-50 transition-colors shadow-sm"
            style={{ fontFamily: PP }}
          >
            <Download size={14} className="text-[#0D47A1]" /> Download Medical
            Report
          </button>
          <button
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {[
          ...
          <div
            key={card.label}
            className="bg-white rounded-2xl border border-[#E5E7EB] p-4 shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border-2 border-[#009688]/30 p-6 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 p-4 rounded-xl bg-slate-50 border border-gray-100 mb-4">
              <div>
                <span
                  className="block text-[11px] text-[#64748B]"
                  style={{ fontFamily: RB }}
                >
                  Doctor
                </span>
            ...
            <div className="p-3.5 rounded-xl bg-blue-50/60 border border-blue-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div
                className="flex items-center gap-2.5 text-xs text-[#0D47A1]"
                style={{ fontFamily: RB }}
              >
                <Building2 size={16} className="text-[#0D47A1]" />
                <span>
                  <strong>OPD Location:</strong> Cardiology Wing A · Room 204
                  (Check-in 10 mins prior)
                </span>
              </div>
              <button
                className="px-3.5 py-1.5 rounded-lg bg-[#0D47A1] text-white text-xs font-semibold hover:bg-[#0c3d8a] transition-colors shrink-0"
                style={{ fontFamily: PP }}
              >
                View OPD Directions
              </button>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-gray-100">
            <button
              className="px-4 py-2 rounded-xl bg-[#009688] text-white text-xs font-bold hover:bg-teal-700 transition-colors"
              style={{ fontFamily: PP }}
            >
              Confirm Attendance
            </button>
            <button
              className="px-4 py-2 rounded-xl border border-[#E5E7EB] text-xs text-[#64748B] font-medium hover:bg-slate-50 transition-colors"
              style={{ fontFamily: RB }}
            >
              Reschedule Slot
            </button>
            <button
              ...
              className="px-4 py-2 rounded-xl border border-[#EF4444]/30 text-xs text-[#EF4444] font-medium hover:bg-red-50 transition-colors ml-auto"
              style={{ fontFamily: RB }}
            >
              Cancel Appointment
            </button>
        ...
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-sm flex flex-col justify-between">
              ...
              {[
                {
                  ...
                  msg: "Cardiology follow-up with Dr. Mehta tomorrow at 10:30 AM",
                  time: "2 hours ago",
                  icon: Calendar,
                  color: "#009688",
                  bg: "bg-teal-50",
                },
                {
                  ...
                  msg: "Atorvastatin & Metoprolol refilled at Main Pharmacy",
                  time: "5 hours ago",
                  icon: Pill,
                  color: "#4DB6AC",
                  bg: "bg-teal-50",
                },
                {
                  ...
                  msg: "Invoice #INV-847 for $97.60 is ready for payment",
                  time: "1 day ago",
                  icon: Receipt,
                  color: "#F59E0B",
                  bg: "bg-amber-50",
                ...
                <div
                  key={idx}
                  className="flex items-start gap-3 p-3 rounded-xl border border-gray-100 bg-slate-50 hover:bg-white transition-colors"
                >
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${notif.bg}`}
                  >
          ...
          <button
            className="mt-3 w-full py-2 rounded-xl border border-[#E5E7EB] text-xs text-[#0D47A1] font-semibold hover:bg-blue-50 transition-colors"
            style={{ fontFamily: PP }}
          >
        ...
        <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm overflow-hidden flex flex-col justify-between">
            ...
            <div className="px-5 py-4 border-b border-gray-50 flex items-center justify-between">
              <div>
                ...
                <p
                  ...
                  style={{ fontFamily: RB }}
                >
                  Pending invoices & payment history
                </p>
              </div>
              <span
                className="text-xs font-bold text-[#F59E0B] bg-amber-50 px-2.5 py-1 rounded-full"
                style={{ fontFamily: PP }}
              >
                $142.60 Total Due
              </span>
              ...
              >
                ...
                <tbody className="divide-y divide-gray-50 text-[#111827]">
                  {[
                    {
                      invoice: "INV-847",
                      desc: "Cardiology OPD Consultation",
                      amount: "$97.60",
                      due: "Due Today",
                      status: "Unpaid",
                    },
                    {
                      invoice: "INV-831",
                      desc: "General Medicine Consultation",
                      amount: "$45.00",
                      due: "Mar 20, 2025",
                      status: "Pending",
                    },
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right">
                        {bill.status !== "Paid" ? (
                          <button
                            className="px-3 py-1 rounded-lg bg-[#0D47A1] text-white text-[11px] font-bold hover:bg-[#0c3d8a] transition-colors"
                            style={{ fontFamily: PP }}
                          >
                            Pay Now
                          </button>
                        ) : (
                          <span className="text-[11px] text-[#66BB6A] font-semibold">
                            ✓ Settled
                          </span>
                        )}
                      </td>
          ...
          <div className="p-4 bg-slate-50 border-t border-gray-100 flex items-center justify-between">
            <span className="text-xs text-[#64748B]" style={{ fontFamily: RB }}>
              2 pending invoices requiring action
            </span>
            <button
              className="px-4 py-2 rounded-xl bg-[#F59E0B] text-white text-xs font-bold hover:bg-amber-600 transition-colors shadow-sm"
              style={{ fontFamily: PP }}
            >
