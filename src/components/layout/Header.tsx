...
interface ActivePatientSelectorProps {
  role: Role;
  currentActive?: FamilyMember;
  familyMembers: readonly FamilyMember[];
  setShowPatientSelector: React.Dispatch<React.SetStateAction<boolean>>;
  setShowProfileMenu: React.Dispatch<React.SetStateAction<boolean>>;
  activePatientName: string;
  showPatientSelector: boolean;
  setPendingSwitchMember: React.Dispatch<
    React.SetStateAction<FamilyMember | null>
  >;
}

type RoleLabelMap = typeof ROLE_LABEL;
...
interface PatientSwitchDialogProps {
  pendingSwitchMember: FamilyMember | null;
  currentActive?: FamilyMember;
  setPendingSwitchMember: React.Dispatch<
    React.SetStateAction<FamilyMember | null>
  >;
  confirmSwitch: (member: FamilyMember) => void;
  isSwitching: boolean;
}

const ActivePatientSelector = ({
  role,
  currentActive,
  familyMembers,
  ...
  activePatientName,
  showPatientSelector,
  setPendingSwitchMember,
}: ActivePatientSelectorProps) => {
  if (role !== "patient" || !currentActive || familyMembers.length === 0)
    return null;
  ...
  return (
    <div className="relative">
      <button
        onClick={() => {
          setShowPatientSelector((v: boolean) => !v);
    ...
    <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl mb-1 border border-slate-100">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div
          ...
          style={{ fontFamily: RB }}
        >
          {(roleLabel as Record<string, string>)[role] || role}
        </div>
...
const PatientSwitchDialog = ({
  pendingSwitchMember,
  currentActive,
  setPendingSwitchMember,
  confirmSwitch,
  isSwitching,
}: PatientSwitchDialogProps) => {
  if (!pendingSwitchMember) return null;
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div
                  ...
                  style={{ fontFamily: PP }}
                >
                  {pendingSwitchMember.patientName || pendingSwitchMember.name}
                </div>
                <div className="text-[10px] text-[#64748B] font-mono">
                  {pendingSwitchMember.mrn} · {pendingSwitchMember.relationship}
                </div>
              </div>
            </div>
          </div>
...
const HeaderToast = ({ toastMsg }: { toastMsg: string | null }) => {
    ...
    <header className="h-16 bg-white border-b border-gray-100 flex items-center px-5 gap-5 shrink-0 z-30 relative">
      ...
      <div className="flex items-center gap-3 ml-auto">
        <ActivePatientSelector
          ...
          setPendingSwitchMember={setPendingSwitchMember}
        />
      ...
      <PatientSwitchDialog
        pendingSwitchMember={pendingSwitchMember}
        currentActive={currentActive}
        setPendingSwitchMember={setPendingSwitchMember}
