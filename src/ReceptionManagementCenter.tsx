...
export interface TodayAppointmentSummary {
  ...
  status: 'Booked' | 'Checked-In' | 'Waiting Nurse' | 'Ready Consultation' | 'In Doctor' | 'Billing' | 'Completed' | 'Cancelled'
  receptionist: string
}
...
const RECEPTION_ACTIVITY_BAR_DATA = [
  { time: '08:00 AM', Bookings: 18, Registrations: 6, CheckIns: 12, Completed: 4 },
  { time: '09:00 AM', Bookings: 32, Registrations: 14, CheckIns: 28, Completed: 16 },
  { time: '10:00 AM', Bookings: 45, Registrations: 18, CheckIns: 39, Completed: 27 },
  { time: '11:00 AM', Bookings: 28, Registrations: 9, CheckIns: 24, Completed: 35 },
  { time: '12:00 PM', Bookings: 14, Registrations: 4, CheckIns: 15, Completed: 22 },
  { time: '01:00 PM', Bookings: 8, Registrations: 2, CheckIns: 10, Completed: 18 }
]
...
function StatusChip({ label, variant }: { label: string; variant: 'success' | 'teal' | 'warning' | 'error' | 'info' | 'default' }) {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold border ${styles[variant]}`} style={{ fontFamily: PP }}>
      {label}
    </span>
  )
}
...
export function ReceptionManagementCenter({ }: { onNavigate?: (screen: string) => void }) {
  ...
  const filteredStaff = useMemo(() => {
    return INITIAL_RECEPTION_STAFF.filter(stf =>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[#64748B] mt-0.5" style={{ fontFamily: RB }}>
            Monitor OPD reception activities, appointment flow, patient check-ins and reception staff performance.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-[#64748B]" style={{ fontFamily: PP }}>Today's Appointments</span>
            <div className="w-9 h-9 rounded-xl bg-blue-50 flex items-center justify-center">
              <Calendar size={18} className="text-[#0D47A1]" />
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-bold text-[#111827]" style={{ fontFamily: PP }}>145</span>
            <span className="inline-flex items-center text-xs font-bold text-[#66BB6A]" style={{ fontFamily: RB }}>
          ...
          <div className="text-xs text-[#64748B] mt-2 pt-2 border-t border-slate-100 flex justify-between" style={{ fontFamily: RB }}>
            ...
            <span className="font-semibold text-[#111827]">145 Appointments</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-[#64748B]" style={{ fontFamily: PP }}>Patients Checked-In</span>
            <div className="w-9 h-9 rounded-xl bg-teal-50 flex items-center justify-center">
              <UserCheck size={18} className="text-[#009688]" />
            </div>
          </div>
          <div className="flex items-baseline justify-between">
            <span className="text-2xl font-bold text-[#111827]" style={{ fontFamily: PP }}>108</span>
            <span className="text-xs font-semibold text-[#009688] bg-teal-50 px-2 py-0.5 rounded-full" style={{ fontFamily: RB }}>
              74.5% Rate
            </span>
          </div>
          <div className="text-xs text-[#64748B] mt-2 pt-2 border-t border-slate-100 flex justify-between" style={{ fontFamily: RB }}>
        ...
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-[#64748B]" style={{ fontFamily: PP }}>Patients Waiting</span>
            <div className="w-9 h-9 rounded-xl bg-amber-50 flex items-center justify-center">
              <Clock size={18} className="text-[#F59E0B]" />
            </div>
          </div>
          <div className="flex items-baseline justify-between">
            <span className="text-2xl font-bold text-[#111827]" style={{ fontFamily: PP }}>24</span>
            <span className="text-xs font-semibold text-[#F59E0B] bg-amber-50 px-2 py-0.5 rounded-full" style={{ fontFamily: RB }}>
              Avg Wait: 14 mins
            </span>
          </div>
          <div className="text-xs text-[#64748B] mt-2 pt-2 border-t border-slate-100 flex justify-between" style={{ fontFamily: RB }}>
            <span>Current Waiting Queue</span>
            <span className="font-semibold text-[#111827]">OPD Lobby A & B</span>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-[#64748B]" style={{ fontFamily: RB }}>
                Comparing bookings, registrations, check-ins, and completed visits
              </p>
            </div>
            <span className="text-xs text-[#64748B]" style={{ fontFamily: RB }}>Hourly Footfall</span>
          </div>

          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={RECEPTION_ACTIVITY_BAR_DATA} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#E5E7EB" />
                <XAxis dataKey="time" tick={{ fontSize: 11, fill: '#64748B' }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fontSize: 11, fill: '#64748B' }} axisLine={false} tickLine={false} />
                <Tooltip
                  contentStyle={{ backgroundColor: '#FFFFFF', borderRadius: '12px', borderColor: '#E5E7EB', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }}
                  labelStyle={{ fontWeight: 'bold', color: '#111827', fontFamily: PP }}
                />
                <Legend wrapperStyle={{ fontSize: '11px', fontFamily: RB, paddingTop: '10px' }} />
                <Bar dataKey="Bookings" fill="#0D47A1" radius={[4, 4, 0, 0]} />
                <Bar dataKey="Registrations" fill="#009688" radius={[4, 4, 0, 0]} />
        ...
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-5 shadow-sm flex flex-col justify-between">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-right">
              <div className="text-xs text-[#64748B]" style={{ fontFamily: RB }}>Total Patients</div>
              <div className="text-lg font-bold text-[#0D47A1]" style={{ fontFamily: PP }}>{TOTAL_QUEUE_PATIENTS}</div>
            </div>
          </div>
          ...
          <div className="h-56 relative flex items-center justify-center my-2">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip
                  contentStyle={{ backgroundColor: '#FFFFFF', borderRadius: '12px', borderColor: '#E5E7EB', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }}
                  itemStyle={{ fontSize: '12px', fontWeight: 'bold', fontFamily: PP }}
                />
              </PieChart>
            </ResponsiveContainer>
            <div className="absolute flex flex-col items-center justify-center pointer-events-none">
              <span className="text-2xl font-bold text-[#111827]" style={{ fontFamily: PP }}>{TOTAL_QUEUE_PATIENTS}</span>
              <span className="text-[10px] text-slate-400 font-medium" style={{ fontFamily: RB }}>Active Queue</span>
            </div>
          </div>
            ...
            {QUEUE_STATUS_DONUT_DATA.map((item) => (
              <div key={item.name} className="flex items-center justify-between p-1.5 rounded-lg bg-slate-50 border border-slate-100">
                <div className="flex items-center gap-1.5 min-w-0">
                  ...
                  <span className="text-[11px] font-medium text-slate-700 truncate" style={{ fontFamily: RB }}>{item.name}</span>
                </div>
                <span className="font-mono text-xs font-bold text-[#111827] ml-1">{item.count}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm overflow-hidden">
        <div className="p-5 border-b border-[#E5E7EB] flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-[#64748B]" style={{ fontFamily: RB }}>
              Operational efficiency, check-in speeds, and throughput by OPD receptionist
            </p>
          </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#E5E7EB] bg-slate-50/70 text-left">
                <th className="px-5 py-3 text-xs font-semibold text-[#64748B]" style={{ fontFamily: RB }}>Receptionist</th>
                <th className="px-5 py-3 text-xs font-semibold text-[#64748B]" style={{ fontFamily: RB }}>Counter</th>
                <th className="px-5 py-3 text-xs font-semibold text-[#64748B]" style={{ fontFamily: RB }}>Appointments Booked</th>
                <th className="px-5 py-3 text-xs font-semibold text-[#64748B]" style={{ fontFamily: RB }}>Patient Registrations</th>
                <th className="px-5 py-3 text-xs font-semibold text-[#64748B]" style={{ fontFamily: RB }}>Check-Ins</th>
                <th className="px-5 py-3 text-xs font-semibold text-[#64748B]" style={{ fontFamily: RB }}>Average Check-In Time</th>
                <th className="px-5 py-3 text-right text-xs font-semibold text-[#64748B]" style={{ fontFamily: RB }}>Actions</th>
              </tr>
            </thead>
                ...
                <tr key={stf.id} className="hover:bg-slate-50/80 transition-colors">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <div className="text-xs font-bold text-[#111827]" style={{ fontFamily: PP }}>{stf.name}</div>
                        <div className="text-[10px] text-slate-400 font-mono">{stf.id}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3.5 text-xs text-[#111827] font-medium" style={{ fontFamily: RB }}>{stf.counter}</td>
                  <td className="px-5 py-3.5 text-xs font-mono font-bold text-[#0D47A1]">{stf.appointmentsBooked}</td>
                  <td className="px-5 py-3.5 text-xs font-mono font-bold text-[#009688]">{stf.patientsRegistered}</td>
                  <td className="px-5 py-3.5 text-xs font-mono font-bold text-[#66BB6A]">{stf.checkInsCompleted}</td>
                  <td className="px-5 py-3.5 text-xs text-[#111827] font-semibold" style={{ fontFamily: RB }}>{stf.avgCheckInTime}</td>
      ...
      <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm overflow-hidden">
        <div className="p-5 border-b border-[#E5E7EB] flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-[#64748B]" style={{ fontFamily: RB }}>
              Real-time snapshot of appointment bookings and receptionist tracking
            </p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#E5E7EB] bg-slate-50/70 text-left">
                <th className="px-5 py-3 text-xs font-semibold text-[#64748B]" style={{ fontFamily: RB }}>Token</th>
                <th className="px-5 py-3 text-xs font-semibold text-[#64748B]" style={{ fontFamily: RB }}>MRN</th>
                <th className="px-5 py-3 text-xs font-semibold text-[#64748B]" style={{ fontFamily: RB }}>Patient</th>
                <th className="px-5 py-3 text-xs font-semibold text-[#64748B]" style={{ fontFamily: RB }}>Doctor</th>
                <th className="px-5 py-3 text-xs font-semibold text-[#64748B]" style={{ fontFamily: RB }}>Department</th>
                <th className="px-5 py-3 text-xs font-semibold text-[#64748B]" style={{ fontFamily: RB }}>Appointment Time</th>
                <th className="px-5 py-3 text-xs font-semibold text-[#64748B]" style={{ fontFamily: RB }}>Current Status</th>
                <th className="px-5 py-3 text-xs font-semibold text-[#64748B]" style={{ fontFamily: RB }}>Receptionist</th>
                <th className="px-5 py-3 text-right text-xs font-semibold text-[#64748B]" style={{ fontFamily: RB }}>Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#E5E7EB]">
              {filteredAppointments.map(apt => (
                <tr key={apt.id} className="hover:bg-slate-50/80 transition-colors">
                  <td className="px-5 py-3.5 font-mono text-xs font-bold text-slate-700 bg-slate-100 px-2 py-0.5 rounded w-fit">{apt.token}</td>
                  <td className="px-5 py-3.5 font-mono text-xs text-slate-500">{apt.mrn}</td>
                  <td className="px-5 py-3.5 text-xs font-bold text-[#111827]" style={{ fontFamily: PP }}>{apt.patientName}</td>
                  <td className="px-5 py-3.5 text-xs text-slate-700 font-medium" style={{ fontFamily: RB }}>{apt.doctor}</td>
                  <td className="px-5 py-3.5 text-xs text-slate-600" style={{ fontFamily: RB }}>{apt.department}</td>
                  <td className="px-5 py-3.5 text-xs text-[#111827] font-semibold" style={{ fontFamily: RB }}>{apt.appointmentTime}</td>
                  <td className="px-5 py-3.5">
                    <StatusChip
                      label={apt.status}
                      variant={
                  ...
                  <td className="px-5 py-3.5 text-xs text-slate-600" style={{ fontFamily: RB }}>{apt.receptionist}</td>
                  <td className="px-5 py-3.5 text-right">
                    <button
            ...
            <div className="py-4 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 bg-slate-50 rounded-xl border border-[#E5E7EB]">
                  <span className="text-[11px] text-[#64748B]" style={{ fontFamily: RB }}>Assigned Counter</span>
                  <div className="text-xs font-bold text-[#111827] mt-0.5" style={{ fontFamily: PP }}>{selectedStaff.counter}</div>
                </div>

                <div className="p-3 bg-slate-50 rounded-xl border border-[#E5E7EB]">
                  <span className="text-[11px] text-[#64748B]" style={{ fontFamily: RB }}>Current Status</span>
                  <div className="mt-0.5">
                    <StatusChip
                      label={selectedStaff.status}
                      variant={selectedStaff.status === 'Active' ? 'success' : selectedStaff.status === 'On Break' ? 'warning' : 'default'}
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="p-3 bg-blue-50 rounded-xl border border-blue-100 text-center">
                  <span className="text-[10px] text-[#0D47A1]" style={{ fontFamily: RB }}>Appointments</span>
                  <div className="text-lg font-bold text-[#0D47A1]" style={{ fontFamily: PP }}>{selectedStaff.appointmentsBooked}</div>
                </div>
                <div className="p-3 bg-teal-50 rounded-xl border border-teal-100 text-center">
                  <span className="text-[10px] text-[#009688]" style={{ fontFamily: RB }}>Registrations</span>
                  <div className="text-lg font-bold text-[#009688]" style={{ fontFamily: PP }}>{selectedStaff.patientsRegistered}</div>
                </div>
                <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-100 text-center">
                  <span className="text-[10px] text-[#66BB6A]" style={{ fontFamily: RB }}>Check-Ins</span>
                  <div className="text-lg font-bold text-emerald-800" style={{ fontFamily: PP }}>{selectedStaff.checkInsCompleted}</div>
                </div>
              </div>

              <div className="p-3 bg-slate-50 rounded-xl border border-[#E5E7EB] flex items-center justify-between text-xs">
                <span className="text-[#64748B]" style={{ fontFamily: RB }}>Average Check-In Speed</span>
                <span className="font-bold text-[#111827]" style={{ fontFamily: PP }}>{selectedStaff.avgCheckInTime} per patient</span>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-2xl max-w-md w-full p-6 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="py-4 space-y-3 text-xs">
              <div className="flex justify-between py-1.5 border-b border-slate-100">
                <span className="text-[#64748B]" style={{ fontFamily: RB }}>Patient Name</span>
                <span className="font-bold text-[#111827]" style={{ fontFamily: PP }}>{selectedAppointment.patientName}</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-slate-100">
                <span className="text-[#64748B]" style={{ fontFamily: RB }}>MRN</span>
                <span className="font-mono text-[#0D47A1] font-bold">{selectedAppointment.mrn}</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-slate-100">
                <span className="text-[#64748B]" style={{ fontFamily: RB }}>Assigned Doctor</span>
                <span className="font-medium text-[#111827]" style={{ fontFamily: RB }}>{selectedAppointment.doctor}</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-slate-100">
                <span className="text-[#64748B]" style={{ fontFamily: RB }}>Department</span>
                <span className="text-[#111827]" style={{ fontFamily: RB }}>{selectedAppointment.department}</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-slate-100">
                <span className="text-[#64748B]" style={{ fontFamily: RB }}>Appointment Time</span>
                <span className="font-bold text-[#111827]" style={{ fontFamily: RB }}>{selectedAppointment.appointmentTime}</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-slate-100">
                <span className="text-[#64748B]" style={{ fontFamily: RB }}>Handled Receptionist</span>
                <span className="text-[#111827]" style={{ fontFamily: RB }}>{selectedAppointment.receptionist}</span>
              </div>
              <div className="flex justify-between items-center py-1.5">
                <span className="text-[#64748B]" style={{ fontFamily: RB }}>Current Status</span>
                <StatusChip
