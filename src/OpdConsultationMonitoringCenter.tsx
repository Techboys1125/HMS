...
import {
  Stethoscope, Users, Clock, CheckCircle2, Search, Filter,
  RotateCcw, Download, Eye, Printer, Activity,
  ArrowUpRight, X, ChevronRight, Building2, UserCheck, BarChart2
} from 'lucide-react'
...
export function OpdConsultationMonitoringCenterScreen({
  ...
  // --- States ---
  const [consultations] = useState<AdminConsultationRecord[]>(ADMIN_CONSULTATIONS)
  ...
  // Modals / Dropdowns
  const [_selectedRecord, _setSelectedRecord] = useState<AdminConsultationRecord | null>(null)
  const [_openDropdownId, _setOpenDropdownId] = useState<string | null>(null)
  ...
  const doctorWorkload = [
    { name: 'Dr. Arjun Mehta', dept: 'Cardiology', assigned: 5, completed: 2, waiting: 2, status: 'In OPD Room 104' },
    { name: 'Dr. Priya Sharma', dept: 'General Medicine', assigned: 2, completed: 1, waiting: 1, status: 'In OPD Room 202' },
    { name: 'Dr. Rajesh Kapoor', dept: 'Neurology', assigned: 2, completed: 0, waiting: 1, status: 'In OPD Room 305' },
  ]
    ...
    <div className="flex-1 bg-[#F1F5F9] overflow-y-auto flex flex-col font-sans">
      ...
      <div className="bg-white border-b border-[#E5E7EB] px-6 py-4">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => alert('Exporting OPD Operational Report (PDF/Excel)')}
      ...
      <div className="p-6 space-y-6 flex-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div className="bg-white rounded-2xl p-4 border border-[#E5E7EB] shadow-sm flex flex-col justify-between">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-[#64748B]" style={{ fontFamily: PP }}>Avg Duration</span>
              <div className="w-9 h-9 rounded-xl bg-purple-50 text-purple-600 flex items-center justify-center">
                <Clock size={18} />
              </div>
            </div>
            <div className="mt-3">
              <div className="text-2xl font-bold text-[#111827]" style={{ fontFamily: PP }}>14 mins</div>
              <div className="text-[11px] text-purple-600 mt-1 font-medium" style={{ fontFamily: RB }}>Target: &lt; 15 mins</div>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-8 xl:col-span-9 space-y-6">
            ...
            <div className="bg-white rounded-2xl p-5 border border-[#E5E7EB] shadow-sm space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 pt-2 border-t border-gray-100">
                <div>
                  <label className="block text-[11px] font-semibold text-[#64748B] mb-1" style={{ fontFamily: PP }}>Date Range</label>
                  <input
                    type="date"
                    value={filterDate}
            ...
            <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm overflow-hidden">
              {isLoading ? (
                ...
                <div className="py-16 px-6 text-center flex flex-col items-center justify-center">
                  ...
                  <p className="text-xs text-[#64748B] max-w-sm mt-1 mb-4" style={{ fontFamily: RB }}>
                      ...
                      <tbody className="divide-y divide-[#E5E7EB] text-xs" style={{ fontFamily: RB }}>
                        {filteredConsultations.map((item) => (
                          <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                            ...
                            <td className="py-3.5 px-4">
                              <span className="px-2 py-0.5 text-[11px] font-semibold rounded bg-blue-50 text-[#0D47A1]" style={{ fontFamily: PP }}>
                                {item.visitType}
                              </span>
                            </td>
                            ...
                            <td className="py-3.5 px-4 font-medium text-slate-700">{item.duration}</td>
                            ...
                            <td className="py-3.5 px-4 text-right relative">
                              <div className="flex items-center justify-end gap-1.5">
                                ...
                                <button
                                  onClick={() => alert(`Printed Operational Summary for ${item.id}`)}
                                  className="p-1.5 hover:bg-slate-200 text-slate-600 rounded-lg transition-colors"
          ...
          <div className="lg:col-span-4 xl:col-span-3 space-y-6">
            ...
            <div className="bg-white rounded-2xl p-5 border border-[#E5E7EB] shadow-sm space-y-4">
              <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                ...
                <span className="text-[10px] font-bold text-[#009688] bg-teal-50 px-2 py-0.5 rounded">Live Monitor</span>
              </div>

              <div className="space-y-2 text-xs" style={{ fontFamily: RB }}>
                <div className="flex justify-between py-1 border-b border-gray-50">
                  <span className="text-slate-500">Total Consultations:</span>
                  <span className="font-bold text-[#111827]" style={{ fontFamily: PP }}>{consultations.length}</span>
                </div>
                <div className="flex justify-between py-1 border-b border-gray-50">
                  <span className="text-slate-500">Completed:</span>
                  <span className="font-bold text-[#66BB6A]" style={{ fontFamily: PP }}>{tabCounts.Completed}</span>
                </div>
                <div className="flex justify-between py-1 border-b border-gray-50">
                  <span className="text-slate-500">Waiting Patients:</span>
                  <span className="font-bold text-amber-600" style={{ fontFamily: PP }}>{tabCounts.Waiting}</span>
                </div>
                <div className="flex justify-between py-1">
                  <span className="text-slate-500">Average Consultation Time:</span>
              ...
              <div className="space-y-2.5 text-xs" style={{ fontFamily: RB }}>
                {doctorWorkload.map((doc, idx) => (
                  <div key={idx} className="p-3 bg-slate-50 border border-slate-100 rounded-xl space-y-1">
                    <div className="flex justify-between font-bold text-[#111827]" style={{ fontFamily: PP }}>
                      <span>{doc.name}</span>
                      <span className="text-[10px] text-[#0D47A1] font-normal">{doc.dept}</span>
                    </div>
                    <div className="flex items-center justify-between text-[11px] text-slate-500">
                      <span>Assigned: <strong>{doc.assigned}</strong> | Done: <strong>{doc.completed}</strong> | Wait: <strong>{doc.waiting}</strong></span>
                    </div>
              ...
              <div className="space-y-2 text-xs" style={{ fontFamily: RB }}>
                {departmentSummary.map((dept, idx) => (
                  <div key={idx} className="flex items-center justify-between p-2.5 bg-slate-50 rounded-xl text-xs">
                    <div>
                      <div className="font-bold text-[#111827]" style={{ fontFamily: PP }}>{dept.dept}</div>
                      <div className="text-[10px] text-slate-500">Done: {dept.completed} · Wait: {dept.waiting}</div>
                    </div>
                    <span className="font-bold text-sm text-[#0D47A1]" style={{ fontFamily: PP }}>
                      {dept.total} Visits
                    </span>
                  </div>
                ))}
      ...
      {showSummaryModal && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 border border-gray-100 shadow-2xl">
            <div className="flex items-center justify-between border-b border-gray-100 pb-3">
              <h3 className="text-base font-bold text-[#111827]" style={{ fontFamily: PP }}>Today's Operational Summary</h3>
              <button onClick={() => setShowSummaryModal(false)} className="p-1 text-slate-400 hover:text-slate-600">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="space-y-3 text-xs" style={{ fontFamily: RB }}>
              <div className="p-3 bg-blue-50 rounded-xl space-y-1">
                <div className="text-[10px] text-blue-600 font-bold uppercase" style={{ fontFamily: PP }}>Total OPD Consultations</div>
                <div className="text-xl font-bold text-[#0D47A1]" style={{ fontFamily: PP }}>8 Consultations Today</div>
              </div>
              ...
              <div className="pt-2 text-slate-500 italic text-[11px]">
                Hospital Administrator operational view — live monitoring data refreshed continuously.
              </div>
            </div>
