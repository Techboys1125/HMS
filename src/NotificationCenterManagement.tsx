...
import {
  ...
  Bell, AlertTriangle, Info, Clock, Shield, Calendar, Users,
  UserCheck, CreditCard, FileText, Settings, Download, X,
  Activity, Pill, MessageSquare, Megaphone, Receipt, DollarSign
} from 'lucide-react'
...
// --- Types ---
export type UserRole = 'Hospital Admin' | 'Doctor' | 'Receptionist' | 'Accountant' | 'Nurse' | 'Patient Portal'
...
const INITIAL_NOTIFICATIONS: NotificationRecord[] = [
  // --- Hospital Admin Notifications ---
  {
    ...
    description: 'Patient Emma Reyes (MRN: P-90823) successfully registered in the system.',
  ...
  },
  {
    ...
    description: '3 consecutive invalid password attempts from IP 192.168.1.112.',
  ...
  },
  {
    ...
    description: 'Patient Sarah Mitchell checked in for Cardiology OPD (Token #08).',
  ...
  },
  {
    ...
    description: 'Patient Robert Chen is waiting in Room 4 for OPD consultation.',
  ...
  },
  {
    ...
    description: 'Prescription RX-8819 successfully finalized for Patient Anita Roy.',
    timestamp: '1 hour ago',
  ...
  },
  {
    ...
    description: 'Your evening OPD shift timing updated to 04:00 PM – 08:00 PM.',
    timestamp: '3 hours ago',
  ...
  // --- Receptionist Notifications ---
  {
    ...
    description: 'Walk-in patient Priya Sharma registered at Front Desk Desk-2.',
  ...
  },
  {
    ...
    description: 'Consultation fee ₹500 pending collection for Patient David Miller.',
    timestamp: '1 hour ago',
    module: 'Billing Module',
    category: 'Billing',
    priority: 'High',
  ...
  },
  {
    ...
    description: 'Invoice INV-1040 has overdue balance of ₹3,200 pending >48 hrs.',
  ...
  },
  {
    ...
    description: 'Refund request #RF-804 submitted for cancelled lab procedure.',
    timestamp: '1 hour ago',
    module: 'Payments Module',
    category: 'Payments',
    priority: 'High',
  ...
  },
  {
    ...
    description: 'Official tax invoice receipt printed for Patient Ramesh Shah.',
    timestamp: '2 hours ago',
    module: 'Invoices Module',
    category: 'Invoices',
    priority: 'Normal',
  ...
  },
  {
    id: 'NOT-ACC-06',
    title: 'Revenue Report Ready',
    description: 'Daily cash register & digital collection reconciliation compiled.',
    timestamp: '3 hours ago',
  ...
  },
  {
    id: 'NOT-NRS-03',
    title: 'Vitals Due',
    description: 'Pre-consultation Blood Pressure & Temperature check pending for Token #12.',
    timestamp: '30 minutes ago',
  ...
  },
  {
    ...
    description: 'Administer prescribed IV booster dose for OPD Day Care Patient #04.',
    timestamp: '45 minutes ago',
  ...
  },
  {
    ...
    description: 'Elevated BP reading (150/95) logged for Patient Vikram Patel.',
    timestamp: '1 hour ago',
    module: 'Clinical Alerts Module',
    category: 'Clinical Alerts',
    priority: 'Critical',
  ...
  // --- Patient Portal Notifications ---
  {
    ...
    description: 'Your OPD appointment with Dr. Arjun Mehta is confirmed for 10:30 AM.',
    timestamp: '15 minutes ago',
    module: 'Appointments Module',
    category: 'Appointments',
    priority: 'Normal',
  ...
  },
  {
    ...
    description: 'Reminder: Please arrive 15 mins prior to your scheduled OPD slot.',
    timestamp: '1 hour ago',
    module: 'Appointments Module',
    category: 'Appointments',
    priority: 'Normal',
  ...
  },
  {
    ...
    description: 'Digital prescription for your recent consultation is ready for download.',
    timestamp: '2 hours ago',
    module: 'Prescriptions Module',
    category: 'Prescriptions',
    priority: 'Normal',
  ...
  },
  {
    ...
    description: 'Dr. Priya Sharma left a follow-up note regarding your dietary regime.',
    timestamp: '6 hours ago',
    module: 'Announcements Module',
    category: 'Announcements',
    priority: 'Normal',
...
const ROLE_QUICK_FILTERS: Record<UserRole, { id: string; title: string; icon: any }[]> = {
  'Hospital Admin': [
...
export function NotificationCenterManagement({
  ...
  const activeQuickFilters = useMemo(() => {
    return ROLE_QUICK_FILTERS[currentRole] || ROLE_QUICK_FILTERS['Hospital Admin']
  }, [currentRole])
  ...
  const filteredNotifications = useMemo(() => {
    return roleNotifications.filter(n => {
      ...
      if (selectedCategory !== 'All' && selectedCategory !== 'Unread' && n.category !== selectedCategory) return false
      ...
      // Department Filter
      if (departmentFilter !== 'All' && n.module !== departmentFilter) return false

      return true
    })
  }, [roleNotifications, selectedCategory, searchQuery, priorityFilter, statusFilter, departmentFilter])

  // Top KPI Metrics
  const kpiMetrics = useMemo(() => {
    const unread = roleNotifications.filter(n => n.status === 'Unread').length
    const today = roleNotifications.filter(n => n.timestamp.includes('minute') || n.timestamp.includes('hour')).length
    const critical = roleNotifications.filter(n => n.priority === 'Critical' || n.priority === 'High').length
    const completed = roleNotifications.filter(n => n.status === 'Completed').length
  ...
  const handleToggleReadStatus = (id: string) => {
    setNotifications(prev => prev.map(n => {
      if (n.id === id) {
        const nextStatus: NotificationStatus = n.status === 'Unread' ? 'Read' : 'Unread'
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'Appointments': return <Calendar className="w-5 h-5 text-blue-600" />
      case 'Patients': return <Users className="w-5 h-5 text-emerald-600" />
      case 'Doctors': return <UserCheck className="w-5 h-5 text-purple-600" />
      case 'Consultations': return <MessageSquare className="w-5 h-5 text-indigo-600" />
      case 'Prescriptions': return <Pill className="w-5 h-5 text-[#009688]" />
      case 'Billing':
      case 'Invoices': return <CreditCard className="w-5 h-5 text-amber-600" />
      case 'Payments':
      case 'Revenue': return <DollarSign className="w-5 h-5 text-emerald-600" />
      case 'Vitals': return <Activity className="w-5 h-5 text-rose-600" />
      case 'Clinical Alerts': return <AlertTriangle className="w-5 h-5 text-red-600" />
      case 'Reports': return <FileText className="w-5 h-5 text-teal-600" />
      case 'Security':
      case 'Audit': return <Shield className="w-5 h-5 text-red-600" />
      case 'System': return <Settings className="w-5 h-5 text-slate-600" />
      case 'Registration': return <UserCheck className="w-5 h-5 text-blue-600" />
      case 'Queue': return <Clock className="w-5 h-5 text-amber-600" />
  ...
  return (
    <div style={{ fontFamily: RB }} className="flex-1 overflow-y-auto p-6 space-y-6 bg-[#F1F5F9] text-[#111827]">
      {/* PAGE HEADER */}
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <p className="text-sm text-[#64748B] mt-0.5">
            View and manage all application notifications, alerts, reminders, and workflow updates relevant to your role.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {(currentRole === 'Hospital Admin' || currentRole === 'Accountant') && (
            <button
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-2xl border border-[#E5E7EB] bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-[#64748B]">Unread Notifications</span>
          ...
          <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-slate-100">
            <div className="h-full bg-[#0D47A1]" style={{ width: `${Math.min(100, kpiMetrics.unread * 15)}%` }} />
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-[#E5E7EB] bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-[#64748B]">Today's Notifications</span>
          ...
          <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-slate-100">
            <div className="h-full bg-[#009688]" style={{ width: `${Math.min(100, kpiMetrics.today * 14)}%` }} />
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-[#E5E7EB] bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-[#64748B]">Priority Alerts</span>
          ...
          <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-slate-100">
            <div className="h-full bg-[#EF4444]" style={{ width: `${Math.min(100, kpiMetrics.critical * 33)}%` }} />
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-[#E5E7EB] bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-[#64748B]">Resolved Actions</span>
          ...
          <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-slate-100">
            <div className="h-full bg-purple-600" style={{ width: `${Math.min(100, kpiMetrics.completed * 25)}%` }} />
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center overflow-x-auto gap-3 pb-2 scrollbar-none">
          {activeQuickFilters.map(item => {
            ...
            return (
              ...
              >
                <IconComp className={`w-4 h-4 ${isActive ? 'text-white' : 'text-[#0D47A1]'}`} />
                <span>{item.title}</span>
                <span
                  className={`ml-1 rounded-full px-2 py-0.5 text-[10px] font-bold ${isActive ? 'bg-white/20 text-white' : 'bg-slate-100 text-[#64748B]'
                    }`}
                >
                  {count}
                </span>
        ...
        <div className="space-y-3">
          {filteredNotifications.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center rounded-2xl border border-[#E5E7EB] bg-white p-12 text-center shadow-sm">
              ...
              <p className="mt-1 text-xs text-[#64748B] max-w-sm">
                Everything is up to date! There are no matching alerts found for your role ({currentRole}).
              </p>
            </div>
          ) : (
            filteredNotifications.map(item => {
              ...
              return (
                <div
                  key={item.id}
                  className={`rounded-2xl border bg-white p-4 shadow-sm transition hover:shadow-md ${isUnread ? 'border-[#0D47A1]/30 bg-blue-50/20' : 'border-[#E5E7EB]'
                    }`}
                >
                    ...
                    <div className="flex items-start gap-3.5">
                      ...
                      <div>
                        ...
                        <div className="mt-2.5 flex items-center gap-3 text-[11px] text-[#64748B]">
                          <span className="font-semibold text-[#009688]">{item.module}</span>
                          <span>•</span>
          ...
          <div className="flex items-center justify-between rounded-2xl border border-[#E5E7EB] bg-white p-4 shadow-sm mt-4">
            <span className="text-xs text-[#64748B]">
              Showing <span className="font-semibold text-[#111827]">1–{filteredNotifications.length}</span> of {filteredNotifications.length} notifications
            </span>
          ...
          <div className="w-full max-w-md bg-white p-6 shadow-2xl flex flex-col justify-between h-full animate-in slide-in-from-right duration-200">
            <div>
              <div className="flex items-center justify-between border-b border-[#E5E7EB] pb-4 mb-5">
                <div>
                  ...
                  <p className="text-xs text-[#64748B]">
                    Configure channel preferences and module alert triggers for {currentRole}.
                  </p>
                </div>
                <button
              ...
              <div className="space-y-4 text-xs">
                <h4 style={{ fontFamily: PP }} className="font-semibold text-[#0D47A1] uppercase tracking-wider text-[11px]">
                  Module Alerts
                </h4>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <input
                    ...
                    onChange={e => setSettings({ ...settings, appointmentNotifs: e.target.checked })}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <input
                    ...
                    onChange={e => setSettings({ ...settings, patientNotifs: e.target.checked })}
                    className="h-4 w-4 rounded border-gray-300 text-[#0D47A1] focus:ring-[#0D47A1]"
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <input
                    ...
                    onChange={e => setSettings({ ...settings, billingNotifs: e.target.checked })}
                    className="h-4 w-4 rounded border-gray-300 text-[#0D47A1] focus:ring-[#0D47A1]"
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <input
                    ...
                    onChange={e => setSettings({ ...settings, reportsNotifs: e.target.checked })}
                    className="h-4 w-4 rounded border-gray-300 text-[#0D47A1] focus:ring-[#0D47A1]"
                  />
                </div>
