...
import {
  Search, Filter, Printer, RefreshCw, ChevronRight,
  Activity, LogIn, Database, Trash2,
  Server, AlertTriangle, CheckCircle2, Info, Eye, X,
  RotateCcw, FileSpreadsheet, FileText, Layers, Clock,
  Lock, Users, ArrowUpRight, ArrowLeft
} from 'lucide-react'
...
const MOCK_AUDIT_RECORDS: AuditRecord[] = [
  ...
  },
  {
    ...
    description: 'Cancelled duplicate draft invoice INV-2026-0892 removed permanently.',
    severity: 'Critical',
    status: 'Success',
    ...
    deletionReason: 'Duplicate draft billing record created accidentally during system sync.'
  },
  {
    id: 'LOG-18962',
    ...
    description: 'Updated emergency contact number and insurance policy number for patient P-10024.',
  ...
  },
  {
    ...
    description: 'Issued e-prescription RX-49201 for Patient P-10088 containing 3 medications.',
    severity: 'Success',
    status: 'Success',
  ...
  },
  {
    ...
    description: 'Invalid password provided 5 consecutive times from unauthorized external IP.',
    severity: 'Critical',
    status: 'Failed',
  ...
  },
  {
    ...
    description: 'Daily financial and operational audit backup snapshot compiled successfully.',
    severity: 'Information',
    status: 'Success',
  ...
  },
  {
    ...
    description: 'Rescheduled Appointment APT-8821 for Dr. Arjun Mehta per patient phone request.',
    severity: 'Information',
    status: 'Success',
  ...
  },
  {
    ...
    description: 'Doctor schedule and OPD consultation fee updated for Dr. Vikram Patel.',
    severity: 'Warning',
    status: 'Success',
    ipAddress: '192.168.1.12',
    device: 'Desktop (macOS Sonoma Admin Workstation)',
  ...
  },
  {
    ...
    description: 'Completed consultation session CON-9041 and saved clinical notes.',
    severity: 'Success',
    status: 'Success',
    ipAddress: '192.168.1.104',
    device: 'Desktop (Windows 11 Workstation)',
  ...
  },
  {
    ...
    description: 'Removed unconfirmed draft booking APT-8810 due to patient no-show.',
    severity: 'Warning',
    status: 'Success',
    ipAddress: '192.168.1.45',
    device: 'Desktop (Windows 10 Reception Terminal)',
  ...
  },
  {
    ...
    description: 'Enforced mandatory 90-day password expiration rule for all clinical staff.',
    severity: 'Warning',
    status: 'Success',
    ipAddress: '192.168.1.12',
    device: 'Desktop (macOS Sonoma Admin Workstation)',
  ...
  },
  {
    ...
    department: 'Administration',
    action: 'User Login Success',
    description: 'Admin console initial daily session established successfully.',
    severity: 'Success',
    status: 'Success',
    ipAddress: '192.168.1.12',
    device: 'Desktop (macOS Sonoma Admin Workstation)',
  ...
  },
  {
    ...
    description: 'Unauthorized access attempt to Restricted Role Management API blocked.',
    severity: 'Critical',
    status: 'Blocked',
...
export default function AuditLogsManagementScreen() {
  ...
  const [currentWorkspace, setCurrentWorkspace] = useState<AuditCategory>('All Logs')
  ...
  const filteredRecords = useMemo(() => {
    return MOCK_AUDIT_RECORDS.filter(record => {
      ...
      if (currentWorkspace !== 'All Logs' && record.category !== currentWorkspace) {
        return false
      }
      ...
      if (selectedRole !== 'All' && record.userRole !== selectedRole) return false
      if (selectedModule !== 'All' && record.module !== selectedModule) return false
      if (selectedUser !== 'All' && record.user !== selectedUser) return false
      if (selectedSeverity !== 'All' && record.severity !== selectedSeverity) return false
      if (selectedStatus !== 'All' && record.status !== selectedStatus) return false
      if (selectedEventType !== 'All' && record.action !== selectedEventType) return false
  ...
  const getKpiCards = () => {
    switch (currentWorkspace) {
      case 'Login History':
        return [
          { title: 'Successful Logins', value: '2,845', color: 'bg-emerald-50 text-emerald-700', trend: '+5.2%', Icon: LogIn },
          { title: 'Failed Logins', value: '142', color: 'bg-red-50 text-red-700', trend: '-2.1%', Icon: AlertTriangle },
          { title: 'Locked Accounts', value: '4', color: 'bg-amber-50 text-amber-700', trend: '0 Today', Icon: Lock },
          { title: 'Active Sessions', value: '88', color: 'bg-blue-50 text-blue-700', trend: 'Live', Icon: Users }
        ]
      case 'User Activities':
        return [
          { title: 'Total Activities', value: '8,326', color: 'bg-purple-50 text-purple-700', trend: '+8.7%', Icon: Activity },
          { title: 'High Priority', value: '312', color: 'bg-amber-50 text-amber-700', trend: '+1.2%', Icon: AlertTriangle },
          { title: "Today's Actions", value: '689', color: 'bg-blue-50 text-blue-700', trend: 'Active', Icon: Clock },
          { title: 'Most Active User', value: 'Sarah J.', color: 'bg-teal-50 text-teal-700', trend: 'Admin', Icon: Users }
        ]
      case 'Data Changes':
        return [
          { title: 'Modified Records', value: '4,281', color: 'bg-teal-50 text-teal-700', trend: '-1.4%', Icon: Database },
          { title: 'Patient Updates', value: '2,140', color: 'bg-blue-50 text-blue-700', trend: '+4.5%', Icon: Users },
          { title: 'Doctor Updates', value: '980', color: 'bg-indigo-50 text-indigo-700', trend: '+0.8%', Icon: Activity },
          { title: 'Billing Changes', value: '1,161', color: 'bg-purple-50 text-purple-700', trend: '+2.0%', Icon: FileText }
        ]
      case 'Deleted Records':
        return [
          { title: 'Records Deleted', value: '842', color: 'bg-amber-50 text-amber-700', trend: '-3.1%', Icon: Trash2 },
          { title: 'Restored', value: '0', color: 'bg-gray-50 text-gray-700', trend: 'Disabled', Icon: RotateCcw },
          { title: 'Permanent Delete', value: '842', color: 'bg-red-50 text-red-700', trend: 'Immutable', Icon: Lock },
          { title: 'Pending Review', value: '12', color: 'bg-blue-50 text-blue-700', trend: 'Admin', Icon: Info }
        ]
      case 'System Logs':
        return [
          { title: 'Services Running', value: '24/24', color: 'bg-emerald-50 text-emerald-700', trend: '100% Operational', Icon: Server },
          { title: 'Background Jobs', value: '1,420', color: 'bg-indigo-50 text-indigo-700', trend: 'Completed', Icon: Activity },
          { title: 'Warning Events', value: '142', color: 'bg-amber-50 text-amber-700', trend: '-0.5%', Icon: AlertTriangle },
          { title: 'Critical Events', value: '18', color: 'bg-red-50 text-red-700', trend: 'Alert', Icon: AlertTriangle }
        ]
      case 'All Logs':
      default:
        return [
          { title: 'Total Audit Events', value: '18,964', color: 'bg-blue-50 text-blue-700', trend: '+12.4%', Icon: Activity },
          { title: 'Successful Logins', value: '2,845', color: 'bg-emerald-50 text-emerald-700', trend: '+5.2%', Icon: LogIn },
          { title: 'User Activities', value: '8,326', color: 'bg-purple-50 text-purple-700', trend: '+8.7%', Icon: Users },
          { title: 'Data Changes', value: '4,281', color: 'bg-teal-50 text-teal-700', trend: '-1.4%', Icon: Database },
          { title: 'Deleted Records', value: '842', color: 'bg-amber-50 text-amber-700', trend: '-3.1%', Icon: Trash2 },
          { title: 'Critical Events', value: '34', color: 'bg-red-50 text-red-700', trend: '+2 Today', Icon: AlertTriangle }
        ]
    }
  }
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-[#F1F5F9]" style={{ fontFamily: RB }}>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
              <span className="hover:text-gray-700 cursor-pointer" onClick={() => setCurrentWorkspace('All Logs')}>Hospital</span>
              <ChevronRight className="w-4 h-4 text-gray-400" />
              <span className="hover:text-gray-700 cursor-pointer" onClick={() => setCurrentWorkspace('All Logs')}>Audit Logs</span>
      ...
      {currentWorkspace === 'All Logs' && (
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              { title: 'Multiple Failed Login Attempts', time: '2026-07-26 11:02:18', module: 'Authentication', desc: '5 failed login attempts detected from IP 185.220.101.5.', severity: 'Critical' as AuditSeverity },
              { title: 'Invoice Cancellation & Deletion', time: '2026-07-26 11:38:02', module: 'Billing', desc: 'Invoice INV-2026-0892 deleted by Admin Sarah Jenkins.', severity: 'Critical' as AuditSeverity },
              { title: 'Permission Denied Alert', time: '2026-07-26 08:10:33', module: 'Authentication', desc: 'Unauthorized access attempt to Restricted Role API blocked.', severity: 'Critical' as AuditSeverity }
            ].map((alert, i) => (
              <div key={i} className="p-4 rounded-xl border border-gray-200 bg-red-50/30 hover:border-red-300 transition-all space-y-2">
                <div className="flex items-center justify-between">
                  {renderSeverityBadge(alert.severity)}
                  <span className="text-[11px] font-mono text-gray-500">{alert.time.split(' ')[1]}</span>
                </div>
                <h4 className="text-xs font-bold text-gray-900">{alert.title}</h4>
                <p className="text-[11px] text-gray-600 line-clamp-2">{alert.desc}</p>
                <div className="pt-2 flex justify-between items-center border-t border-gray-100">
                  <span className="text-[11px] font-semibold text-blue-900">{alert.module}</span>
                  <button
                    onClick={() => setActiveDetailsRecord(MOCK_AUDIT_RECORDS[4])}
      ...
      <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b border-gray-100 pb-3">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Select an audit workspace to open dedicated log records, metrics, and filter streams.
            </p>
          </div>
            ...
            return (
              ...
              >
                <div>
                  ...
                  <h4 className={`text-sm font-bold ${isActive ? 'text-white' : 'text-gray-900'}`} style={{ fontFamily: PP }}>
                    {card.title}
                  </h4>
                  <p className={`text-[11px] mt-1 line-clamp-2 leading-relaxed ${isActive ? 'text-blue-100' : 'text-gray-500'}`}>
                    {card.description}
                  </p>
                </div>
      ...
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200">
        ...
        <div className="mt-4 pt-4 border-t border-gray-100">
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
            ...
            <div>
              ...
              >
                ...
                <option value="System Automated Job">System Automated Job</option>
              </select>
            </div>
            ...
            <div>
              ...
              >
                ...
                <option value="Multiple Failed Login Attempts">Failed Logins</option>
                <option value="Patient Details Modified">Data Changes</option>
                <option value="Cancelled Invoice Deleted">Deleted Records</option>
          ...
          <div className="flex items-center justify-end gap-3 pt-3 mt-3 border-t border-gray-100">
            ...
            <button
              onClick={() => showToast(`Applied filters. Showing ${filteredRecords.length} records.`)}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-4 border-b border-gray-200 bg-gray-50/50 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-gray-900" style={{ fontFamily: PP }}>
            ...
            <div className="py-16 text-center space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 max-w-sm mx-auto mt-1">
                  No audit records match the selected filters in {currentWorkspace}.
                </p>
              </div>
              <button
                onClick={resetFilters}
                    ...
                    <tr key={rec.id} className="hover:bg-blue-50/40 transition-colors group">
                      ...
                      {currentWorkspace === 'All Logs' && (
                        <>
                          ...
                          <td className="p-3.5 whitespace-nowrap">{renderSeverityBadge(rec.severity)}</td>
                          <td className="p-3.5 whitespace-nowrap">{renderStatusBadge(rec.status)}</td>
                        </>
                      )}
                      ...
                      {currentWorkspace === 'User Activities' && (
                        <>
                          ...
                          <td className="p-3.5 font-semibold text-blue-950">{rec.action}</td>
                          <td className="p-3.5 max-w-xs truncate text-gray-500" title={rec.description}>
                            {rec.description}
                          </td>
                          <td className="p-3.5 whitespace-nowrap text-gray-500 font-mono">{rec.timestamp}</td>
                          <td className="p-3.5 whitespace-nowrap">{renderStatusBadge(rec.status)}</td>
                      ...
                      {currentWorkspace === 'Deleted Records' && (
                        <>
                          ...
                          <td className="p-3.5 whitespace-nowrap font-bold text-gray-900">{rec.user}</td>
                          <td className="p-3.5 text-gray-600 max-w-xs truncate" title={rec.deletionReason || rec.description}>
      ...
      <div className="sticky bottom-0 z-30 bg-white/95 backdrop-blur border-t border-gray-200 px-6 py-3 rounded-2xl shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="text-xs font-semibold text-gray-700">
          Workspace: <span className="text-blue-900 font-bold">{currentWorkspace}</span> ({filteredRecords.length} of 18,964 Records)
        </div>
